...
public class BluetoothDevicePairer {
    ...
    public static final boolean REMOTE_HAS_IR = false;
    ...
    private static final String AVAILABLE_DEVICE_NAME = "RemoteB009" ;
    private static final String AVAILABLE_GOOGLE_SMALL_DEVICE_NAME = "RemoteG10" ;
    private static final String AVAILABLE_GOOGLE_LARGE_DEVICE_NAME = "RemoteG20" ;
	private static final String AVAILABLE_DMG_DEVICE_NAME = "TATV Remote" ;
	private static final String AVAILABLE_CNS_DEVICE_NAME = "ATV RC" ;
    ...
    private final BluetoothScanner.Listener mBtListener = new BluetoothScanner.Listener() {
        ...
        public void onDeviceAdded(BluetoothScanner.Device device) {
            if (DEBUG) {
                Log.d(TAG, "Scan device: " + device.btDevice.getName() + " - " + device.btDevice.getAddress());
            }

            //if (AVAILABLE_CNS_DEVICE_NAME.contains(device.btDevice.getName()))
			if(device.btDevice.getName().contains(AVAILABLE_CNS_DEVICE_NAME))
            {
                if (DEBUG) {
                    Log.d(TAG, "Adding device: " + device.btDevice.getAddress());
                }

                if (!mVisibleScannerDevices.contains(device)) {
                    mVisibleScannerDevices.add(device);
                }
        ...
        public void onDeviceRemoved(BluetoothScanner.Device device) {
            ...
            mVisibleScannerDevices.remove(device);
    ...
    public static boolean hasValidInputDevice(Context context, int[] deviceIds) {
        ...
        for (int ptr = deviceIds.length - 1; ptr > -1; ptr--) {
            ...
            if (device == null)
                return false;
			Log.d(TAG, "valid input device: "+device);
            ...
            if (!device.isVirtual() && device.isExternal() && isCompatible) {
                return true;
            }
        }
        return false;
    ...
    private final ArrayList<BluetoothScanner.Device> mVisibleScannerDevices = new ArrayList<>();
    ...
    public List<BluetoothScanner.Device> getAvailableScannerDevices() {
        if (DEBUG) Log.d(TAG,"getAvailableScannerDevices");
        ArrayList<BluetoothScanner.Device> copy = new ArrayList<>(mVisibleScannerDevices.size());
        copy.addAll(mVisibleScannerDevices);
        return copy;
    }
    ...
    private void onDeviceFound(BluetoothDevice device) {
        if (!mVisibleDevices.contains(device)) {
            ...
            // one situation is old state 10 => new state 11, but next is old state 11 => new state 10
            // this unexpected error will cause pair stuck if we return here.
            Log.d(TAG, "STATUS_ERROR happend, need refresh state, to make sure next round pair can be work !!!!!");
            // return;
        }
    ...
    private void onBondFailed() {
        ...
        if (mVisibleScannerDevices.size() > 1) {
            mVisibleScannerDevices.remove(0);
        }
    }
