...
public class ServerControlService extends Service
{
    ...
    public static boolean checkServerConfig = true;
    ...
    public static final String ABUPDATE_BROADCAST_FORCE_UPDATE     = "prime.abupdate.force.update";
    public static final int    ABUPDATE_INTERNET_MODE              = 0;

    // OTA Updater package name
    public static final String OTA_UPDATER_PACKAGE_NAME         ="com.prime.otaupdater";
    ...
    // property
    public static final String PROPERTY_CONFIG_URL  = "persist.sys.prime.server_control_config";
    public static final String SET_NETWORK_IS_READY = "persist.sys.abupdate.internet.ready";//set from otaservice

    // default value
    // public static final String DEFAULT_CONFIG_URL   = "http://10.1.4.180:8080/default.json";
    public static final int    DEFAULT_DOWNLOAD_CONFIG_CYCLE = 30 * 1000;
        ...
        public void handleMessage(@NonNull Message msg) {
            switch (msg.what) {
                ...
                case MSG.WHITELIST_GET:
                    ...
                    break;
                case MSG.BROADCAST_OTA_ERROR:
                    Log.i(TAG, "MSG.BROADCAST_OTA_ERROR");
                    checkService();
                    break;
    ...
    public void onCreate() {
        ...
        initConfigURL();
        registerOtaReceiver();
        //registerAppManagerReceiver(); remove AppManager
        initNetworkInfoLis();
    }
    ...
    public void onDestroy() {
        ...
        //unregisterReceiver(mAppManagerReceiver); remove AppManager
        ...
        mNetworkCallback = new ConnectivityManager.NetworkCallback() {
            ...
            public void onAvailable(Network network) {//Connect Network, but not check internet ready
                ...
                try {
                    ...
                    Log.d(TAG, "onAvailable Network Connection success NetworkReady = [TRUE]");
                } catch (Exception e) {
                    Log.d(TAG, "onAvailable Network Connection fail NetworkReady = [FALSE]");
                }
            }
            ...
            public void onLost(Network network) {//Disconnect Network
                ...
                Log.d(TAG, "onLost Network DisConnect  ===>> NetworkReady = [FALSE]");
            }
        };
    }
    ...
    public boolean isServiceRunning() {
        ...
        //boolean isAppManager = false; remove AppManager
        boolean isOtaService = false;

        if (manager != null) {
            for (ActivityManager.RunningServiceInfo service : manager.getRunningServices(Integer.MAX_VALUE)) {
                String serviceClassName = service.service.getClassName();
                //if (serviceClassName.equals("com.prime.appDownloadManager.ServerCommunicate.BackendManagementService")) remove AppManager
                //    isAppManager = true; remove AppManager
                if (serviceClassName.equals("com.prime.android.tv.otaservice.DownloadService"))
                    isOtaService = true;
            }
        }
        //Log.d(TAG, "isServiceRunning: " + (isAppManager && isOtaService)); remove AppManager
        //return isAppManager && isOtaService; remove AppManager
        Log.d(TAG, "isServiceRunning: " + isOtaService);
        return isOtaService;
    }
    ...
    public void configDownloadAgain() {
        ...
        Config.DOWNLOAD_CONFIG_CYCLE = 5000;
    ...
    public void configDownload() {
        ...
        Log.d(TAG, "configDownload: Delay     : " + Config.DOWNLOAD_CONFIG_CYCLE + " Ms ................................................................");
        ...
        }).start(), Config.DOWNLOAD_CONFIG_CYCLE);
    }
    ...
    public void configUpdate(Object object) {
        ...
        // Configuration Download Config Cycle
        try {
            configuration       = new JSONObject((String) object);
            Config.DOWNLOAD_CONFIG_CYCLE = configuration.getLong("Configuration Download Config Cycle") * 1000;
        }
        catch (JSONException e) {
            Log.e(TAG, "configUpdate: Error Configuration Download Config Cycle");
            Config.DOWNLOAD_CONFIG_CYCLE = DEFAULT_DOWNLOAD_CONFIG_CYCLE;
        }
        // Configuration URL
        try {
        ...
        try {
            ...
            Config.OTA_FORCE_UPDATE       = otaFile.getBoolean("force update");
        }
        catch (JSONException e) {
        ...
        if (Config.OTA_PAYLOAD_BIN        != null &&
            ...
            //sendMessage(MSG.WHITELIST_ON_OFF); remove AppManager
        }
    }
    ...
    public void configLog() {
        ...
        Log.d(TAG, "configLog: DOWNLOAD_CONFIG_CYCLE    " + Config.DOWNLOAD_CONFIG_CYCLE);
        ...
        Log.d(TAG, "configLog: OTA_FORCE_UPDATE         " + Config.OTA_FORCE_UPDATE);
        /*  remove AppManager
    ...
    */}
    ...
    public void broadcastOTA(boolean timestampOK) {
        ...
        String metaData     = Config.OTA_METADATA;
        Boolean forceUpdate = Config.OTA_FORCE_UPDATE;
        int    TODO_MSG     = MSG.WHITELIST_ON_OFF;

        if (OtaReceiver.OTA_IN_PROGRESS) {
            Log.e(TAG, "broadcastOTA: OTA in progress");
            //sendMessage(TODO_MSG); remove AppManager
            return;
        }
        ...
        Log.d(TAG, "broadcastOTA: START , metadata      = " + metaData);
        Log.d(TAG, "broadcastOTA: START , forceUpdate   = " + forceUpdate);
        ...
        /* send to otaupdater */
        Intent intent = new Intent(ACTION_HTTP_OTA_UPDATE);
        intent.setPackage(OTA_UPDATER_PACKAGE_NAME);
        intent.putExtra(EXTRA_HTTP_PAYLOAD_BIN, payloadBin);
        intent.putExtra(EXTRA_HTTP_PAYLOAD_PROP, payloadProp);
        intent.putExtra(EXTRA_HTTP_METADATA, metaData);
        intent.putExtra(EXTRA_HTTP_FORCE, forceUpdate);
