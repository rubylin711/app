...
public class TVTunerParams implements Parcelable {
    ...
    public TVTunerParams (int tuner_id,int tpId, int fe_type){
    ...
    public static TVTunerParams CreateTunerParamDVBT(int tuner_id, int tpId, int frequency, int bandwith){
        TVTunerParams tp = new TVTunerParams(tuner_id,tpId, FE_TYPE_DVBT);
        tp.setFrequency(frequency);
        tp.setBandwith(bandwith);
        return tp;
    }
    public static TVTunerParams CreateTunerParamDVBS(int tuner_id, int tpId, int frequency, int symbolRate, int polar){
        TVTunerParams tp = new TVTunerParams(tuner_id,tpId, FE_TYPE_DVBS);
        tp.setFrequency(frequency);
        tp.setSymbolRate(symbolRate);
        tp.setPolar(polar);
        return tp;
    }
    public static TVTunerParams CreateTunerParamDVBC(int tuner_id, int tpId, int frequency, int symbolRate, int Qam){
        TVTunerParams tp = new TVTunerParams(tuner_id,tpId, FE_TYPE_DVBC);
        tp.setFrequency(frequency);
        tp.setSymbolRate(symbolRate);
        tp.setQam(Qam);
        return tp;
