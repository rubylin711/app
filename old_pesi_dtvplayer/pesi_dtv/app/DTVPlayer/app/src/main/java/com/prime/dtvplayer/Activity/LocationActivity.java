...
public class LocationActivity extends DTVActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        cityList = getResources().getStringArray(R.array.STR_ARRAY_CITY_LIST);
        provinceCity = (Spinner) findViewById(R.id.provinceCityTXVSPINNER);
        AreaText = (TextView) findViewById(R.id.menuWideAreaTXV);
        RegionText = (TextView) findViewById(R.id.menuLocationTXV);

        InitTitleHelp();

        AddAllCity();

        City = new SelectBoxView(this, provinceCity, cityList);
        provinceCity.setOnItemSelectedListener(CityListener);

        int curLocation = 0;
        VMXProtectData data = GetProtectData();
        cityNode curNode ;
    ...
    private void InitTitleHelp ()
    {
        ...
        ActivityTitleView title = (ActivityTitleView) findViewById( R.id.TitleLayout );
        ActivityHelpView help = (ActivityHelpView) findViewById( R.id.HelpLayout );

        title.setTitleView( getString( R.string.STR_LOCATION_SETTINGS_TITTLE ) );

        help.setHelpInfoTextBySplit( "" );
    ...
    @Override
    public void onBackPressed() {//Scoty 20181218 modify VMX location rule
        int pos = City.GetSelectedItemIndex();
        SetProtectData(location_list.get(pos).first_hex,location_list.get(pos).second_hex,location_list.get(pos).third_hex);
    ...
    public void AddAllCity()
    {
        ...
        //AddCity(0x00, 0x00, 0x00, "Undefined");//Scoty 20181218 modify VMX location rule
    }
    ...
    private AdapterView.OnItemSelectedListener CityListener =  new SelectBoxView.SelectBoxOnItemSelectedListener()
    {
        ...
        public void onItemSelected(AdapterView adapterView, View view, int position, long id) {
            ...
            if(location_list != null && position < location_list.size()) {
                ...
                Log.d(TAG, "onItemSelected:  Area =" + Area + "      Region =" + Region);
    ...
    {
        Log.d(TAG, "getLoactionAreaStr: firstHex =" + firstHex);
        int i;
        ...
        for(i=0; i <location_list.size(); i++)
        {
            ...
            if(city.first_hex == firstHex && city.second_hex == 0 && city.third_hex == 0)
            {
                ...
                for( int j = 0; j < str.length; j++)
                    Log.d(TAG, "getLoactionAreaStr:  str  " + j + "   ==>" + str[j] + "       str.length =" + str.length + "     city.cityname =" + city.cityname);
                if(str.length >= 4) {
                    Log.d(TAG, "getLoactionAreaStr:  str[3] =" + str[3]);
                    location = str[3];
                    return location;
                }
                else
        ...
        return getResources().getString(R.string.STR_ERROR);
    }
