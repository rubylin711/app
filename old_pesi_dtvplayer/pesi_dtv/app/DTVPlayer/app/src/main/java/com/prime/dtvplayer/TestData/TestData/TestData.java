package com.prime.dtvplayer.TestData.TestData;
...
public class TestData {
    ...
    void SetTestDataProgramInfo(int TunerType)
    {
        ...
        if(TunerType == TpInfo.DVBC || TunerType == TpInfo.DVBT)
        {
            //Program 0
...
//        final int Enable_NO = 1;
    ...
    private void SetTestDataGposInfo()
    {
        ...
        mTestDataGposInfo.setDBVersion("1.0");
        ...
        mTestDataGposInfo.setAudioTrackMode(0);
    ...
    public static void AddEPGEvent(List<EPGEvent> list, int durationH, int durationM, int durationS, int epgType) // Add EPGEvent by pre EPGEvent
    {
        ...
        long preStartTime = list.get(prePos).getStartTime();
        long preEndTime = list.get(prePos).getEndTime();
        long duration = (durationH*60*60 + durationM*60 +durationS)*1000;
...
//        epgEvent.setDuration( GetLongByHMS(durationH, durationM, durationS) );
        epgEvent.setStartTime(preEndTime);
        epgEvent.setDuration(duration);
        epgEvent.setEndTime(preEndTime+duration);
    ...
    private void SetTestDataEPGEvent()
    {
        ...
        int daysAfter;
        long curTime = Calendar.getInstance().getTime().getTime()-(30*60*1000);
        long middleDuration = 3600*1000; //one hour
        long smallDuration = 1800*1000; //one hour
        long largeDuration = 7200*1000; //one hour
...
//        epgEvent.setDuration( GetLongByHMS(0, 5, 0) );
        epgEvent.setStartTime(curTime);
        epgEvent.setDuration(middleDuration);
        epgEvent.setEndTime(curTime+middleDuration);
        epgEvent.setParentalRate(0);
        epgEvent.setShortEvent("Short Event " + epgEvent.getEventId());
...
//        }
    ...
    */
    ...
    public long GetTestDataDefaultChannelChanneId() {
