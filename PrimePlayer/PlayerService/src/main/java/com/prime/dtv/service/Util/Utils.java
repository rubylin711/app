...
public final class Utils {
    ...
    public static int getInt(final byte[] bytes, final int offset, final int len, final int mask){
        if (bytes == null) {
            Log.w(TAG, "getInt out of bounds: buf is null");
            return 0;
        }
        if (len <= 0) {
            return 0;
        ...
        if (offset < 0 || offset + len > bytes.length) {
            Log.w(TAG, "getInt out of bounds: bufLen=" + bytes.length + " offset=" + offset + " len=" + len);
            return 0;
        }
        int r=0;
        for (int i = 0; i < len; i++) {
    ...
    public static DiskInfo find_disk_by_id(Context context, String diskId) {
        StorageManager sm = (StorageManager) context.getSystemService(Context.STORAGE_SERVICE);
        DiskInfo disk = null;
        try {
            @SuppressLint("DiscouragedPrivateApi")
            Method findDiskById = StorageManager.class.getDeclaredMethod("findDiskById", String.class);
            disk = (DiskInfo) findDiskById.invoke(sm, diskId);
        }
        catch (Exception e) {
            Log.e(TAG, "findDiskById: fail", e);
        }
