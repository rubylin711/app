...
public class NitData extends TableData {
    ...
    public boolean check_one_seg_service(int serviceID, int onID){
        for(final TransportStream tStream:mtransportStreamList)
        {
            List<DescBase> descBase_list = tStream.mDescriptor.getDescriptorList(ISDBT_PARTIAL_RECEPTION_DESC);
            if(!descBase_list.isEmpty())
            {
                for(DescBase temp : descBase_list)
                {
                    ISDBTPartialReceptionDescriptor tempDescriptor = (ISDBTPartialReceptionDescriptor) temp;
                    for (ISDBTPartialReceptionDescriptor.ServiceID sid : tempDescriptor.msidList) {
                        Log.d(TAG, "sid.serviceID:"+sid.serviceID+","+serviceID);
                        if (sid.serviceID == serviceID && tStream.mOriginalNetworkID == onID) {
                            return true;  // 找到目標 ServiceID，返回 true
                        }
                    }
                }
            }
    ...
    public int getISDBT_remote_id(){
        for(final TransportStream tStream:mtransportStreamList)
        {
            List<DescBase> descBase_list = tStream.mDescriptor.getDescriptorList(TS_INFORMATION_DESC);
            if(!descBase_list.isEmpty())
            {
                TSInformationDescriptor tempDescriptor = (TSInformationDescriptor) descBase_list.get(0);
                return tempDescriptor.remote_control_key_id;
            }
        }
        Log.w(TAG, "not match find TS_INFORMATION_DESC, return 0");
        return -1;
    }
