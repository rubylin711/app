...
public class LocalTimeOffsetDescriptor extends DescBase{
    ...
    private List<LocalTimeOffset> mLocalTimeOffsetList = new ArrayList<>();
    ...
    public void Parsing(byte[] data, int lens) {
        ...
        while (t<Length) {
            ...
            LocalTimeOffset s = new LocalTimeOffset(countryCode, countryRegionId, localTimeOffsetPolarity, localTimeOffset, timeOfChange_msb, timeOfChange_lsb, nextTimeOffset);
    ...
    public class LocalTimeOffset {
        ...
        public int getLocalTimeOffsetMinutes() {
            Date nextTimeOffsetDate = get_utc_time(TimeOfChangeMsb,TimeOfChangeLsb);
            Date nowDate = new Date();
            int hr,min,offsetMinute = 0;
            if(nowDate.getTime() > nextTimeOffsetDate.getTime()) {
                hr = bcd_to_bin(nextTimeOffset)/100;
                min = bcd_to_bin(nextTimeOffset)%100;
            }
            else {
                hr = bcd_to_bin(LocalTimeOffset)/100;
                min = bcd_to_bin(LocalTimeOffset)%100;
            }
            offsetMinute = hr*60 + min;
            offsetMinute = (LocalTimeOffsetPolarity == 0) ? offsetMinute : (0-offsetMinute);
            Log.d(TAG,"timeOffset = "+offsetMinute+ " minutes");
            return offsetMinute;
        }
    }
