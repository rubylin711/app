...
//    }
    ...
    private String getKeyValue(String key,String defaultValue) {
        ...
        Cursor cursor;
        try {
            cursor = mContext.getContentResolver().query(CONTENT_URI, new String[]{GposDatabaseTable.KEY, GposDatabaseTable.VALUE}, GposDatabaseTable.KEY + " = ?", new String[]{key}, null);
    ...
    private void setKeyValue(String key, long value) {
        ...
        try {
            mContext.getContentResolver().update(CONTENT_URI, Value, where, null);
        }
        catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void save(GposInfo gposInfo) {
        ...
        setKeyValue(GposInfo.GPOS_PURCHASE_PASSWORD_VALUE,gposInfo.getPurchasePasswordValue()+"");
        ...
        setKeyValue(GposInfo.GPOS_TUNER_OUTPUT_5V, gposInfo.getTunerOutput5V()+"");
        ...
        for(int i =0 ; i<GposInfo.MAX_NUM_OF_GROUP; i++){
            String key = GposInfo.GPOS_BouquetIds+i;
            setKeyValue(key, gposInfo.getBouquetIds(i)+"");
        }
        for(int i =0 ; i<GposInfo.MAX_NUM_OF_GROUP; i++){
            String key = GposInfo.GPOS_BouquetVers+i;
            setKeyValue(key, gposInfo.getBouquetVers(i)+"");
        }
        for(int i =0 ; i<GposInfo.MAX_NUM_OF_GROUP; i++){
            String key = GposInfo.GPOS_BouquetNames+i;
            setKeyValue(key, gposInfo.getBouquetNames(i)+"");
        }
        setKeyValue(GposInfo.GPOS_AREA_CODE, gposInfo.getAreadCode()+"");
        setKeyValue(GposInfo.GPOS_CM_MODE, gposInfo.getCmMode()+"");
        setKeyValue(GposInfo.GPOS_TUTORIALSETTING, gposInfo.getTutorialSetting()+"");
        setKeyValue(GposInfo.GPOS_STBDATA_RETURN, gposInfo.getSTBDataReturn()+"");
        setKeyValue(GposInfo.GPOS_HDMI_CEC, gposInfo.getHdmiCecOnOff()+"");
        setKeyValue(GposInfo.GPOS_RESOLUTION, gposInfo.getResolution()+"");
        setKeyValue(GposInfo.GPOS_CAS_UPDATE_TIME, gposInfo.getCasUpdateTime()+"");*/
    }
...
//            dbver = "S";
        ...
        gposInfo.setWorkerPasswordValue(Integer.parseInt(getKeyValue(GposInfo.GPOS_WORKER_PASSWORD_VALUE,"13197776")));
        gposInfo.setPurchasePasswordValue(Integer.parseInt(getKeyValue(GposInfo.GPOS_PURCHASE_PASSWORD_VALUE,"0000")));
        ...
        gposInfo.setZipCode(getKeyValue(GposInfo.GPOS_ZIPCODE,"0000"));
        ...
        gposInfo.setSo(getKeyValue(GposInfo.GPOS_SO,"0"));
        ...
        gposInfo.setTunerOutput5V(Integer.parseInt(getKeyValue(GposInfo.GPOS_TUNER_OUTPUT_5V,"0")));
        for(int i =0 ; i<GposInfo.MAX_NUM_OF_IRD_COMMAND; i++){
        ...
        gposInfo.setAreaCode(getKeyValue(GposInfo.GPOS_AREA_CODE, "0"));
        gposInfo.setCmMode(Integer.parseInt(getKeyValue(GposInfo.GPOS_CM_MODE, "0")));
        gposInfo.setTutorialSetting(Integer.parseInt(getKeyValue(GposInfo.GPOS_TUTORIALSETTING, "0")));
        gposInfo.setSTBDataReturn(Integer.parseInt(getKeyValue(GposInfo.GPOS_STBDATA_RETURN, "1")));
        gposInfo.setHdmiCecOnOff(Integer.parseInt(getKeyValue(GposInfo.GPOS_HDMI_CEC, "1")));
        gposInfo.setResolution(Integer.parseInt(getKeyValue(GposInfo.GPOS_RESOLUTION, "0")));
        gposInfo.setCasUpdateTime(Long.parseLong(getKeyValue(GposInfo.GPOS_CAS_UPDATE_TIME, "0")));
        processGposInfo(gposInfo);
        LogUtils.d("STBRefCasDataTime = "+gposInfo.getSTBRefCasDataTime());*/
        return gposInfo;
    }

    private HashMap<String, String> loadAllKeyValues() {
        HashMap<String, String> dataMap = new HashMap<>();
        Cursor cursor = null;
        try {
            cursor = mContext.getContentResolver().query(
                    CONTENT_URI,
                    new String[]{GposDatabaseTable.KEY, GposDatabaseTable.VALUE},
                    null, null, null
            );

            if (cursor != null && cursor.moveToFirst()) {
                do {
                    String key = mDbUtils.GetStringFromTable(cursor, GposDatabaseTable.KEY);
                    String value = mDbUtils.GetStringFromTable(cursor, GposDatabaseTable.VALUE);
    ...
    private void processGposInfo( GposInfo gposInfo ) {
        HashMap<String, String> keyValueMap = loadAllKeyValues();
        ArrayList<ContentValues> insertList = new ArrayList<>();

        for (int i = 0; i < GposInfo.MAX_NUM_OF_IRD_COMMAND; i++) {
            String key = GposInfo.GPOS_IrdCommand + i;
            String value = keyValueMap.getOrDefault(key, "0");
            GposInfo.setIrdCommand(mContext, i, Long.parseLong(value));

            if (!keyValueMap.containsKey(key)) {
                ContentValues values = new ContentValues();
                values.put(GposDatabaseTable.KEY, key);
                values.put(GposDatabaseTable.VALUE, "0");
                insertList.add(values);
            }
        }

        for (int i = 0; i < GposInfo.MAX_NUM_OF_GROUP; i++) {
            String BouquetIds_key = GposInfo.GPOS_BouquetIds + i;
            String BouquetVers_key = GposInfo.GPOS_BouquetVers + i;
            String BouquetNames_key = GposInfo.GPOS_BouquetNames + i;

            String BouquetIds_value = keyValueMap.getOrDefault(BouquetIds_key, "-1");
            String BouquetVers_value = keyValueMap.getOrDefault(BouquetVers_key, "255");
            String BouquetNames_value = keyValueMap.getOrDefault(BouquetNames_key, "");

            GposInfo.setBouquetIds(mContext, i, Integer.parseInt(BouquetIds_value));
            GposInfo.setBouquetVers(mContext, i, Integer.parseInt(BouquetVers_value));
            gposInfo.setBouquetNames(mContext, i, BouquetNames_value);

            if (!keyValueMap.containsKey(BouquetIds_key)) {
                ContentValues values = new ContentValues();
                values.put(GposDatabaseTable.KEY, BouquetIds_key);
                values.put(GposDatabaseTable.VALUE, "-1");
                insertList.add(values);
            }
            if (!keyValueMap.containsKey(BouquetVers_key)) {
                ContentValues values = new ContentValues();
                values.put(GposDatabaseTable.KEY, BouquetVers_key);
                values.put(GposDatabaseTable.VALUE, "255");
                insertList.add(values);
            }
            if (!keyValueMap.containsKey(BouquetNames_key)) {
                ContentValues values = new ContentValues();
                values.put(GposDatabaseTable.KEY, BouquetNames_key);
                values.put(GposDatabaseTable.VALUE, "");
                insertList.add(values);
            }
        }

        if (!insertList.isEmpty()) {
            ContentValues[] bulkInsertArray = new ContentValues[insertList.size()];
            mContext.getContentResolver().bulkInsert(CONTENT_URI, insertList.toArray(bulkInsertArray));
        }
    }
}
