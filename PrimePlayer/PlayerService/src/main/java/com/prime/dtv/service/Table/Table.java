            ...
            public void onPesiFilterEvent(byte[] data, int dataLen) {
                ...
                if(tableData!=null){
                    if (mTableId != PMT_TABLE_ID) {
                        if (mMultiTable == true) {
                            if(mTableId == SDT_OTHER_TS_TABLE_ID || mTableId == SDT_ACTUAL_TS_TABLE_ID) {
                            int TsId, TsId2,i;
                            ...
                            if(mTableId == BAT_TABLE_ID){
                                int bouquetId,i,bouquetId2;
                                bouquetId = ((BatData) tableData).getBouquetId();
                                for (i = 0; i < mDataList.size(); i++) {
                                    bouquetId2 = ((BatData) mDataList.get(i)).getBouquetId();
                                    if (bouquetId2 == bouquetId) {
                                        mDataList.remove(i);
                                        break;
                                    }
                                }
