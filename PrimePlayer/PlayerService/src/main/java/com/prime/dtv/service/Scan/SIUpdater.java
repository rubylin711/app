...
public class SIUpdater extends Updater{
        ...
        while(true) {
            try {
                ...
                if(Pvcfg.getModuleType().equals(Pvcfg.LAUNCHER_MODULE.LAUNCHER_MODULE_DMG)) {
                    ...
                    Pesi_Monitor_SI_CNS(programInfo);
                }
                else{
    ...
    private int Pesi_Check_SI_DMG(GposInfo gposInfo, ProgramInfo programInfo, NitData nitData, List<SdtData> sdtDataList, int check_allupdate_flag) {
        ...
        //NetVer = GposInfo.getNitVersion(mContext);
        //NetId = GposInfo.getSINitNetworkId(mContext);
        TunerId = Pvcfg.PROJECT_HOME_TP_TUNER_ID;//programInfo.getTunerId();

        if (nitData != null) {
            if(nitData.getNetworkStreamList().size() > 0){
                NetVer=GposInfo.getNitVersion(mContext);//mNitData.getNetworkStreamList().get(0).getVersion();
                    ...
                    for (DescBase temp : descBaseListTmp) {
                        ...
                        if (tempDescriptor.LinkageType == 0xCA){
                            ...
                            if(!WvCasUrlDatas.isEmpty()){
                                ...
                                LogUtils.d("mStbUpdateTime = "+WvCasUrlDatas.get(0).mStbUpdateTime+" "+GposInfo.getSTBRefCasDataTime(mContext));
                                if(!WvCasUrlDatas.get(0).mUrl.equalsIgnoreCase(GposInfo.getWVCasLicenseURL(mContext)) ||
                                        WvCasUrlDatas.get(0).mStbUpdateTime != GposInfo.getSTBRefCasDataTime(mContext)){
 ...
 */
                    if (update_flag != 0) {
                        ...
                        mScanUtils.Scan_process_Nit(NitData,mCallback);
                    ...
                    */
                        ...
                        if (update_flag > 1) {
                            List<MusicInfo> musicInfoList = ServiceInterface.get_prime_dtv().getACSMusicList();
...
//                        mDataManager.DataManagerSaveData(DataManager.SV_FAV_INFO);
    ...
    private int Pesi_Check_SI_TBC(GposInfo gposInfo, ProgramInfo programInfo, NitData nitData, BatData batData,List<SdtData> sdtDataList){
        ...
        if(batData != null){
            int Verion;
            BatVer=GposInfo.getBatVersion(mContext);
    ...
    private int Pesi_Check_SI_CNS(GposInfo gposInfo, ProgramInfo programInfo, NitData nitData, BatData batData,List<SdtData> sdtDataList){
        int i,j,size,count,NetVer, NetId, TunerId,nit_update_flag=0,sdt_update_flag=0,bat_update_flag=0,update_flag=0;
        Nit nit;
        Sdt sdt;
        List<TpInfo> tpInfoList=null;
        ...
        if (nitData != null) {
            if(nitData.getNetworkStreamList().size() > 0){
                ...
                if(NAGRA_NETWORK_ID != NetId) {
                    ...
                    update_flag=0;
                    LogUtils.d("SI_CNS Network id is NAGRA_NETWORK_ID(0x31)");
                    return update_flag;
                }
            }
        }
        ...
        if(batData != null){
            ...
            if (Verion != 0xff) {
                ...
                bat_update_flag = 1;
            if(bat_update_flag == 1){
                LogUtils.d("SI_CNS set BAT ID to Gpos "+batData.getBouquetId());
                GposInfo.setBatId(mContext, batData.getBouquetId());
            }
        }
        ...
        LogUtils.d("SI_CNS sdt_update_flag = "+sdt_update_flag+" nit_update_flag = "+nit_update_flag+" update_flag = "+update_flag);
        return update_flag;
    }
        ...
        synchronized(mSiUpdateDmgLock) {
            ...
            try {
                ...
                while (check_allupdate_flag < 2) {
                    ...
                    GposInfo_BatId=GposInfo.getBatId(mContext);
                    if(Pvcfg_BatId != GposInfo_BatId){
                        ...
                        GposInfo.setNitVersion(mContext, 0xff);
                        GposInfo.setBatVersion(mContext, 0xff);
                    }
...
//                        mDataManager.DataManagerSaveData(DataManager.SV_FAV_INFO);
                        ...
                        GposInfo.setBatId(mContext, Pvcfg.getBatId());
                    }
                    check_allupdate_flag = 2;
                }
                reset_si_table(nit, NitData, sdt, SdtDataList, bat, batData);
    ...
    private int Pesi_Monitor_SI_CNS(ProgramInfo programInfo){
        int update_flag=0,check_allupdate_flag;
        List<ProgramInfo> programInfoList=new ArrayList<>();
        List<ProgramInfo> currProgramInfoList;
        ProgramInfo programInfoTmp;
        ...
        synchronized(mSiUpdateDmgLock) {
            ...
            List<SdtData> SdtDataList = null;
            ProgramInfo programInfoOld = new ProgramInfo(programInfo);
            LogUtils.d("SI_CNS Lcn = "+programInfo.getLCN()+" Name = "+programInfo.getDisplayName());
            if(mTuneInterface.isLock(TunerId) == false){
                LogUtils.d("SI_CNS Tuner is no lock, TunerId = "+TunerId);
            }
            else{
                LogUtils.d("SI_CNS Tuner is lock, TunerId = "+TunerId);
            }
            //long goToChannelbyChId=-1;
            try {
                check_allupdate_flag = 0;
                ...
                while (check_allupdate_flag < 2) {
                    if (IsStop() == true) {
                        reset_si_table(nit, NitData, sdt, SdtDataList, bat, batData);
                        LogUtils.d("SI_CNS Pesi_Monitor_SI Stop");
                        break;
                    }
                    LogUtils.d("SI_CNS Pesi_Monitor_SI IN, check_allupdate_flag = "+check_allupdate_flag);
                    nit = new Nit(TunerId, true);
                    nit.processWait();
                    NitData = nit.getNitData();
                    if (NitData != null) {
                    ...
                    if (IsStop() == true /*|| SdtDataList == null*/ || NitData == null) {
                        LogUtils.e("SI_CNS Pesi_Monitor_SI Stop IsStop() = "+IsStop()+" SdtDataList = "+SdtDataList+" NitData = "+NitData);
                        reset_si_table(nit, NitData, sdt, SdtDataList, bat, batData);
                        break;
                    }
                    Pvcfg_BatId=Pvcfg.getBatId();
                    GposInfo_BatId=GposInfo.getBatId(mContext);
                    if(Pvcfg_BatId != GposInfo_BatId){
                        LogUtils.d("SI_CNS Pvcfg_BatId != GposInfo_BatId, Pvcfg_BatId = "+Pvcfg_BatId+" GposInfo_BatId = "+GposInfo_BatId);
                        GposInfo.setNitVersion(mContext, 0xff);
                        GposInfo.setBatVersion(mContext, 0xff);
                    }

                    if(Pvcfg_BatId != 0) {
                        LogUtils.d("SI_CNS Get Bat Table Pvcfg_BatId = "+Pvcfg_BatId);
                        bat = new Bat(TunerId, Pvcfg_BatId, false);
                        bat.processWait();
                        batData = bat.getBatData();
                    }
//                    LogUtils.d("SI_CNS SdtDataList = "+SdtDataList);
                    if(SdtDataList != null)
                        mScanUtils.find_ticker_ad_serviceId_CNS(programInfoOld,SdtDataList);
                    update_flag = Pesi_Check_SI_CNS(mDataManager.getGposInfo(), programInfo, NitData, batData, SdtDataList);

                    if (IsStop() == true) {
                        LogUtils.e("SI_CNS Pesi_Monitor_SI Stop");
                        reset_si_table(nit, NitData, sdt, SdtDataList, bat, batData);
                        break;
                    }
                    LogUtils.d("SI_CNS update_flag = "+update_flag);
                    if (update_flag != 0) {
                        if (!(check_allupdate_flag == 7) && check_allupdate_flag == 0) {
                            check_allupdate_flag = 1;
                            reset_si_table(nit, NitData, sdt, SdtDataList, bat, batData);
...
//                                Log.d("SI_CNS","new p["+p.getDisplayNum()+"]"+
//                                        " Name = "+p.getDisplayName() + " freq = "+tp.CableTp.getFreq()
//                                    + " SID = "+p.getServiceId() + " channel id = "+p.getChannelId());
//                            }
//                        }
//                        {
//                            for(ProgramInfo p : currProgramInfoList) {
//                                TpInfo tp = mDataManager.getTpInfo(p.getTpId());
//                                Log.d("SI_CNS","old p["+p.getDisplayNum()+"]"+
//                                        " Name = "+p.getDisplayName() + " freq = "+tp.CableTp.getFreq()
//                                        + " SID = "+p.getServiceId() + " channel id = "+p.getChannelId());
//                            }
//                        }
                        ...
                        LogUtils.d("SI_CNS currProgramInfoList number = "+currProgramInfoList.size());
//                        {
//                            for(ProgramInfo p : currProgramInfoList) {
//                                TpInfo tp = mDataManager.getTpInfo(p.getTpId());
//                                Log.d("SI_CNS","compare p["+p.getDisplayNum()+"]"+
//                                        " Name = "+p.getDisplayName() + " freq = "+tp.CableTp.getFreq()
//                                        + " SID = "+p.getServiceId());
//                            }
//                        }
                        programInfoTmp = mScanUtils.FindServiceByTripleID(currProgramInfoList, programInfo);
//                        Log.d("SI_CNS","cur program = "+"["+
//                                programInfo.getDisplayNum()+"] "+programInfo.getDisplayName());
                        need_to_setchannel = false;

                        if (programInfoTmp != null) {
//                            LogUtils.d("SI_CNS programInfoTmp != NULL");
                            if (programInfoTmp.getTpId() != programInfo.getTpId())
                                need_to_setchannel = true;
                            if (programInfoTmp.getSkip() == 0) {
                                if (need_to_setchannel == true) {
...
//                            LogUtils.d("SI_CNS programInfoTmp == NULL");
                        ...
                        if(need_notify_ui){
                            LogUtils.e("SI_CNS Need Notify UI to get new channel list");
                            mCallback.sendCallbackMessage(DTVMessage.PESI_EVT_SYSTEM_SI_UPDATE_CHANNEL, 1, 0, null);
                        }
                        if (need_to_setchannel) {
                            LogUtils.e("SI_CNS Need change channel");
                            mCallback.sendCallbackMessage(DTVMessage.PESI_EVT_SYSTEM_SI_UPDATE_SET_1ST_CHANNEL, 0, 1, programInfoTmp);
                        }
                        else {
                            LogUtils.d(" isE301 "+isE301);
                            if(isE301){
                                LogUtils.e("SI_CNS, unblock E301, set Current channel");
                                isE301 = false;
                                mCallback.sendCallbackMessage(DTVMessage.PESI_EVT_SYSTEM_SI_UPDATE_SET_CHANNEL, 0, 1, programInfoTmp);
                            }else {
                                LogUtils.d("SI_CNS Don't need change channel");
                            }
                        }
                        GposInfo.setBatId(mContext, Pvcfg.getBatId());
                    }
