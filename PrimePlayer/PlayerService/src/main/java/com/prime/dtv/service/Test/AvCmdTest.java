...
public class AvCmdTest {
    ...
    public void playAv() {
        ...
        TpInfo tp = new TpInfo(TpInfo.DVBC,dataManager.mTpInfoList.size(),0,1,482000,5200,TpInfo.Cable.QAM_256,0);
...
//        }
        for(int i = 0; i < dataManager.getProgramInfoList().size();i++) {
            ProgramInfo programInfo = dataManager.getProgramInfoList().get(i);
            if(programInfo.getChannelId() == GposInfo.getCurChannelId(mContext)) {
