...
public class DemuxSectionCallback {
    ...
    // Table ID	+ Section syntax indicator + Private bit + Reserved bits + Section length
    ...
    private final List<Integer> mMutiBouquetIdId = new ArrayList<>();
    ...
    public void setMultiBouquetId(int Count) {
        int i;
        for(i=0;i<Count;i++){
            mMutiBouquetIdId.add(0);
            mSectionNumberList.add(new LinkedList<>());
            mLastSectionNumberList.add(0);
            mSectionRepeatedCountList.add(0);
        }
                    ...
                    if (event instanceof SectionEvent) {
                        ...
                        if (!mArrivedSectionNumberList.contains(mSectionNumber)) {
                            ...
                            if (mSectionRepeatedCount > (mLastSectionNumber + 1)) {
                                ...
                                //Log.d(TAG,"too much repeated sections !!, mSectionRepeatedCount = "+mSectionRepeatedCount);
                            }
                        }
                    }
                }
                ...
                for (FilterEvent event : events) {
                    ...
                    if (event instanceof SectionEvent) {
                        ...
                        if(Pvcfg.isCheckSectionComplete() == true)
                        {
                            ...
                            //Log.d(TAG, "onFilterEvent: " + Arrays.toString(buffer));
                            //Log.d(TAG, "onFilterEvent: sectionLength = " + sectionLength);
                            //Log.d(TAG, "Eit onFilterEvent: lastSectionNumber = " + mLastSectionNumber+" mSectionNumber = "+mSectionNumber);
                            //Log.d(TAG,"Eit mArrivedSectionNumberList = "+mArrivedSectionNumberList.toString());
                            // if new arrived section number
                            if (!mArrivedSectionNumberList.contains(mSectionNumber)) {
                ...
                for (FilterEvent event : events) {
                    ...
                    if (event instanceof SectionEvent) {
                        ...
                        if(mMultiTable){
                            ...
                            //Log.d(TAG, "onFilterEvent: mMutiTransportStreamId.size() = " + mMutiTransportStreamId.size());
                            //Log.d(TAG, "onFilterEvent: mSectionNumberList.size() = " + mSectionNumberList.size());
                            //Log.d(TAG, "onFilterEvent: mLastSectionNumberList.size() = " + mLastSectionNumberList.size());
                            //Log.d(TAG, "onFilterEvent: mSectionRepeatedCountList.size() = " + mSectionRepeatedCountList.size());
                            ...
                            for (i = 0; i < size; i++) {
                                //Log.d(TAG, "onFilterEvent: mMutiTransportStreamId.get(" + i + ") = " + mMutiTransportStreamId.get(i));
                            ...
                            //Log.d(TAG, "onFilterEvent: LastSectionNumberList = " + Byte.toUnsignedInt(buffer[7]));
                            //Log.d(TAG, "onFilterEvent: mLastSectionNumberList.get(" + i + ") = " + mLastSectionNumberList.get(i));
                            //Log.d(TAG, "onFilterEvent: mSectionNumber = " + mSectionNumber);
                            //Log.d(TAG, "onFilterEvent: mSectionNumberList.get(" + i + ") = " + mSectionNumberList.get(i));
                            if (!mSectionNumberList.get(i).contains(mSectionNumber)) {
                                ...
                                if (mSectionNumberList.get(i).size() > mLastSectionNumberList.get(i)) {
                                    /* complete the whole table, exit */
                                    //Log.d(TAG, "onFilterEvent: Remove mMutiTransportStreamId.get(" + i + ") = " + mMutiTransportStreamId.get(i));
                                    //Log.d(TAG, "Remove mMutiTransportStreamId.get(" + i + ") = " + mMutiTransportStreamId.get(i));
                                ...
                                //Log.d(TAG, "onFilterEvent: mSectionRepeatedCountList.get(" + i + ") = " + mSectionRepeatedCountList.get(i));
                                //Log.d(TAG, "onFilterEvent: mLastSectionNumberList.get(" + i + ") = " + mLastSectionNumberList.get(i));
...
*/
    ...
    private FilterCallback getBatMultiFilterCallback() {
        return new FilterCallback() {
            @Override
            public void onFilterEvent(Filter filter, FilterEvent[] events) {
                for (FilterEvent event : events) {
                    ...
                    if (event instanceof SectionEvent) {
                        ...
                        int version;// = sectionEvent.getVersion();
                        int dataLength = sectionEvent.getDataLength();
                        int i,size,sectionLength,bouquetId;
                        ...
                        bouquetId = Utils.getInt(buffer, 3, 2, Utils.MASK_16BITS);
                        tableId = Utils.getInt(buffer, 0, 1, Utils.MASK_8BITS);
                        //LogUtils.d("@@@@@@ tableId = " + tableId + ", bouquetId = " + bouquetId);
                        //Log.d(TAG, "onFilterEvent: " + Arrays.toString(buffer));
                        //Log.d(TAG, "onFilterEvent: tableId = " + Integer.toString(tableId,16));
                        //Log.d(TAG, "onFilterEvent: transportStreamId = " + transportStreamId);
                        //Log.d(TAG, "onFilterEvent: mMutiTransportStreamId.size() = " + mMutiTransportStreamId.size());
                        ...
                        //Log.d(TAG, "onFilterEvent: mLastSectionNumberList.size() = " + mLastSectionNumberList.size());
                        //Log.d(TAG, "onFilterEvent: mSectionRepeatedCountList.size() = " + mSectionRepeatedCountList.size());
                        if ((mMutiBouquetIdId.size() == mSectionNumberList.size() &&
                                //mMutiTransportStreamId.size() == mSectionNumberList.size() &&
                                mMutiBouquetIdId.size() == mLastSectionNumberList.size() &&
                                mMutiBouquetIdId.size() == mSectionRepeatedCountList.size()) == false) {
                            //(TAG, "onFilterEvent: List Size error");
                            mFlagReceived = 0;
                            break;
                        }
                        size = mMutiBouquetIdId.size();
                        for (i = 0; i < size; i++) {
                            //Log.d(TAG, "onFilterEvent: mMutiTransportStreamId.get(" + i + ") = " + mMutiTransportStreamId.get(i));
                            if (bouquetId == mMutiBouquetIdId.get(i)) {
                                break;
                            }
                        }
                        if (i >= size) {
                            ...
                            if (i >= size) {
                                continue;
                                    ...
                                    mMutiBouquetIdId.set(i, bouquetId);
                            }
                        }
                        //mSectionNumber = sectionEvent.getSectionNumber();
                        mLastSectionNumberList.set(i, Byte.toUnsignedInt(buffer[7]));
                        ...
                        if (!mSectionNumberList.get(i).contains(mSectionNumber)) {
                                ...
                                if (mSectionNumberList.get(i).size() > mLastSectionNumberList.get(i)) {
                                    ...
                                    //Log.d(TAG, "onFilterEvent: Remove mMutiTransportStreamId.get(" + i + ") = " + mMutiTransportStreamId.get(i));
                                    //Log.d(TAG, "Remove mMutiTransportStreamId.get(" + i + ") = " + mMutiTransportStreamId.get(i));
                                    mMutiBouquetIdId.remove(i);
                                    mLastSectionNumberList.remove(i);
                                    mSectionRepeatedCountList.remove(i);
                                    mSectionNumberList.remove(i);
                                    //Log.d(TAG, "onFilterEvent: mMutiTransportStreamId.size() = " + mMutiTransportStreamId.size());
                                    if (mMutiBouquetIdId.size() == 0) {
                                        //Log.d(TAG, "onFilterEvent: mFlagReceived = " + mFlagReceived);
                                        mFlagReceived = 0;
                                    }
                                }
            ...
            public void onFilterEvent(Filter filter, FilterEvent[] events) {
                for (FilterEvent event : events) {
                    ...
                    if (event instanceof SectionEvent) {
                        ...
                        //Log.e(TAG, "chuck DSMCC: section in");
    ...
    private void createFilterCallback(byte tableId) {
        ...
        if (tableId == Table.TDT_TABLE_ID) {
            ...
            //Log.d(TAG, "getSdtOtherFilterCallback tableId = " +Integer.toString(tableId,16) );
        ...
        else if (tableId == Table.BAT_TABLE_ID) {
            //Log.e(TAG,"@@@@@@ createFilterCallback, getBatMultiFilterCallback()");
            mFilterCallback = getBatMultiFilterCallback();
        }
