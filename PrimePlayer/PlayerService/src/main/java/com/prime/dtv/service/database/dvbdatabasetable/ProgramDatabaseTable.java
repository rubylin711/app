...
public class ProgramDatabaseTable {
    ...
    public ProgramDatabaseTable(Context context) {
        ...
        CONTENT_URI = Uri.parse(
                "content://" + DVBContentProvider.getDVBContentProvider(context).getAuthority() + "/" + TABLE_NAME);
    }
    ...
    public static String TableCreate() {
        String cmd = "create table "
                ...
                + ProgramInfo.TV_INPUT_CHANNEL_URI
                + ");";
        return cmd;
    }
    ...
    public static final DVBDatabase.SQLiteBinder<ProgramInfo> BINDER = (stmt, programInfo) -> {
        ...
        if (programInfo.getTvInputChannelUri() != null)
            stmt.bindString(i++, programInfo.getTvInputChannelUri().toString());
        else
            i++;
    };
    ...
    public void save(List<ProgramInfo> programInfoList) {
        ...
        // Log.d("qqqq", "program list delete time = " + (t1 - t0) + " ms, all = " + (t2
        // - t0) + " ms");
    }
    ...
    private ContentValues getProgramInfoContentValues(ProgramInfo programInfo) {
        ...
        if (programInfo.getTvInputChannelUri() != null)
            Value.put(ProgramInfo.TV_INPUT_CHANNEL_URI, programInfo.getTvInputChannelUri().toString());
        // Log.d(TAG, "getProgramInfoContentValues "+ programInfo.ToString());
        return Value;
    }

    private void add(ProgramInfo programInfo) {
        mContext.getContentResolver().insert(CONTENT_URI, getProgramInfoContentValues(programInfo));
    }

    private void update(ProgramInfo programInfo) {
        String selection = ProgramInfo.CHANNEL_ID + " = " + programInfo.getChannelId();
        ContentValues values = getProgramInfoContentValues(programInfo);
        try (Cursor cursor = mContext.getContentResolver().query(CONTENT_URI, null, selection, null, null)) {
            if (cursor != null && cursor.getCount() > 0) {
                mContext.getContentResolver().update(CONTENT_URI, values, selection, null);
            } else {
                mContext.getContentResolver().insert(CONTENT_URI, values);
            }
        } catch (Exception e) {
            Log.e(TAG, "update program failed: " + programInfo.getChannelId(), e);
        }
    }
    ...
    public int removeAll() {
        ...
        count = mContext.getContentResolver().delete(CONTENT_URI, null, null);
    ...
    public void remove(ProgramInfo programInfo) {
        ...
        count = mContext.getContentResolver().delete(CONTENT_URI, where, null);
    }
    ...
    private List<ProgramInfo> queryList() {
        ...
        for (i = 0; i < programInfoList.size(); i++) {
            Log.d(TAG, "[" + i + "] " + programInfoList.get(i).getDisplayName() + " channel id = "
                    + programInfoList.get(i).getChannelId());
        }
    ...
    // Cursor cursor =
    // mContext.getContentResolver().query(DtvChannelTable.CONTENT_URI, null,
    // selection, null, null);
    ...
    private ProgramInfo ParseCursor(Cursor cursor) {
        ...
        programInfo.setParentCountryCode(
                mDbUtils.GetStringFromTable(TABLE_NAME, cursor, ProgramInfo.PARENTAL_COUNTRY_CODE));
        programInfo.pVideo = programInfo.getVideoInfoFromJsonString(
                mDbUtils.GetStringFromTable(TABLE_NAME, cursor, ProgramInfo.VIDEO_INFO_JSON));
        programInfo.pAudios = programInfo.getAudioInfoFromJsonString(
                mDbUtils.GetStringFromTable(TABLE_NAME, cursor, ProgramInfo.AUDIO_INFO_JSON));
        programInfo.pSubtitle = programInfo.getSubtitleInfoFromJsonString(
                mDbUtils.GetStringFromTable(TABLE_NAME, cursor, ProgramInfo.SUBTITLE_INFO_JSON));
        programInfo.pTeletext = programInfo.getTeletextInfoFromJsonString(
                mDbUtils.GetStringFromTable(TABLE_NAME, cursor, ProgramInfo.TELETEXT_INFO_JSON));
        ...
        String UriStr = mDbUtils.GetStringFromTable(TABLE_NAME, cursor, ProgramInfo.TV_INPUT_CHANNEL_URI);
        if (UriStr != null && !UriStr.isEmpty())
            programInfo.setTvInputChannelUri(Uri.parse(UriStr));
