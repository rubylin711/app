...
public class PmtData extends TableData {
    ...
    private static int SUBTITLE_C608       = 0x04;//dvb_subtitle
    private static int SUBTITLE_ARIB_STD_B24 = 0x05;
...
//        private int ca_system_id;
        ...
        public int getTtxt_init_page() {
            return ttxt_init_page;
        }
...
//        }
    ...
    public class ElementaryStreamInformation {
        ...
        public int	elementary_stream_pid;
        //private int[][] lang = new int[2][4];//lang[0] => Left ,lang[1]=>Right;
        private int ttype;
        private int C608_flag;
         ...
         */
        // --- DSM-CC related ---
        public boolean isDsmcc;          // true if stream_type 0x0B/0x0D or has 0x66
        public int dsmccType;            // 0x0B (Object carousel) / 0x0D (U-N messages)
        public int dataBroadcastId = -1; // from descriptor 0x66
        public byte[] dataBroadcastSelector; // raw selector bytes (運營商私有)
        public long carouselId = -1;     // from descriptor 0x13
        public int associationTag = -1;  // from descriptor 0x14
        public int componentTag = -1;    // from descriptor 0x52 (可對應 AIT)
        private int tdata;
...
//    }
    public Program_map getProgramMap(){
        if (mProgramMap == null) {
            Log.w(TAG, "Program_map is null; returning empty Program_map");
            mProgramMap = new Program_map();
        }
        return mProgramMap;
    }
...
//    }
    ...
    public void buildesiList(byte[] data, int offset, int lens) {
        ...
        for (i = offset; i < lens;) {
            ...
            // === NEW: detect DSM-CC by stream_type ===
            if (esiData.stream_type == 0x0B || esiData.stream_type == 0x0D) {
                esiData.isDsmcc = true;
                esiData.dsmccType = esiData.stream_type;
                esiData.tdata = 0x66; // 讓後面的 buildpmtList() 能把它當 DSM-CC PID 串到 update_pid
            }
            ...
            for(j=0; j<es_info_length; j+=toUnsignedInt(data[array_shift+j+1])+2){
                ...
                int len = toUnsignedInt(data[array_shift + j + 1]);
                ...
                switch (toUnsignedInt(data[array_shift + j])) {
                    case TELETEXT_DESC:
                    {
                        ...
                        for(l=0; l < des_length; l+= 5)
                        {
                            ...
                            //Log.d(TAG, String.format("Teletext Entry [%d]: type=0x%02X", l / 5, ttxt_type));
                            switch(ttxt_type)
                            {
                                case 0x01: // Initial Teletext Page
                                  //Log.d(TAG, "Teletext Init Page (type 0x01)");
                                  break;
                              case 0x02: // Subtitle Page
                                  //Log.d(TAG, "Teletext Subtitle Page (type 0x02)");
                                  break;
                              case 0x05: // Hearing Impaired
                                  //Log.d(TAG, "Teletext Subtitle for Hearing Impaired (type 0x05)");
                                  break;
                              default:
                                  Log.d(TAG, String.format("    ??Unknown Teletext Type: 0x%02X", ttxt_type));
                                  break;
                                }
		                    String lang = getISO8859_1String(data, ttxt_shift + l, 3);
                            int mag = toUnsignedInt(data[ttxt_shift + l + 3]) & 0x07;
                            int page = toUnsignedInt(data[ttxt_shift + l + 4]);
                            int fullPage = mag * 100 + page;
                    
                            esiData.ttxt_lang[ttxt_lang_number] = lang;
                                    esiData.ttxt_type[ttxt_lang_number] = ttxt_type;
                            esiData.ttxt_magazine_number[ttxt_lang_number] = mag;
                            esiData.ttxt_page_number[ttxt_lang_number] = page;
                    
                            Log.d(TAG, String.format("TtxParse entry:%d type:%d Lang: %s, Mag: %d, Page: %d (Full Page: %d)",
                                    l/5,ttxt_type,lang, mag, page, fullPage));

                                    ttxt_lang_number ++;
                                    esiData.ttxt_lang_num = ttxt_lang_number;
                            esiData.ttype = TELETEXT_SUBTITLE;  // mark as subtitle regardless of type
                    ...
                    case 0x7a://eric lin 20090220 support DD+
                        esiData.ac3_flag = 2;
                        break;
                    // === NEW: Data Broadcast ID Descriptor (0x66) ===
                    case 0x66: {
                        // descriptor_tag(1) length(1) data_broadcast_id(2) + selector_bytes(len-2)
                        int off = array_shift + j + 2;
                        int dbid = ((toUnsignedInt(data[off]) << 8) | toUnsignedInt(data[off + 1])) & 0xFFFF;
                        esiData.dataBroadcastId = dbid;
                        esiData.isDsmcc = true;
                        esiData.tdata = 0x66; // 與你原架構相容

                        if (len > 2) {
                            byte[] sel = new byte[len - 2];
                            System.arraycopy(data, off + 2, sel, 0, len - 2);
                            esiData.dataBroadcastSelector = sel; // 運營商自定義可留存
                        }
                        // 可選：Log.d(TAG, String.format("DSM-CC dbid=0x%04X pid=0x%04X", dbid, esiData.elementary_stream_pid));
                        break;
                    }

                    // === NEW: Carousel Identifier Descriptor (0x13) ===
                    case 0x13: {
                        // descriptor_tag(1) length(1) carousel_id(4) [big-endian]
                        if (len >= 4) {
                            int off = array_shift + j + 2;
                            long cid = ((long)toUnsignedInt(data[off]) << 24) |
                                       ((long)toUnsignedInt(data[off + 1]) << 16) |
                                       ((long)toUnsignedInt(data[off + 2]) << 8) |
                                       ((long)toUnsignedInt(data[off + 3]));
                            esiData.carouselId = cid;
                            esiData.isDsmcc = true;
                            esiData.tdata = 0x66;
                        }
                        break;
                    ...
                    // === NEW: Association Tag Descriptor (0x14) ===
                    case 0x14: {
                        // association_tag(2) use(2) selector_length(1) selector_bytes ... private_data_length(1) private_data ...
                        int off = array_shift + j + 2;
                        if (len >= 4) {
                            int assoc = ((toUnsignedInt(data[off]) << 8) | toUnsignedInt(data[off + 1])) & 0xFFFF;
                            esiData.associationTag = assoc;
                            esiData.isDsmcc = true;
                            esiData.tdata = 0x66;
                        }
                        break;
                    ...
                    case 0x52:
                    {
                        if(esiData.stream_type == STREAM_TELETEXT)
                        {
                            int des_length;
                            des_length = toUnsignedInt(data[array_shift+j+1]);
                            if(des_length >= 1 )
                            {
                                int component_tag = toUnsignedInt(data[array_shift+j+2]);
                    ...
                    case DATA_COMPONENT_DESC: {
                        if(esiData.stream_type == STREAM_TELETEXT) {
                            desc_length = toUnsignedInt(data[array_shift + j + 1]);
                            int data_component_start = array_shift + j;
                            byte[] desc_array = Arrays.copyOfRange(data, data_component_start, data_component_start + desc_length + 2);
                            DataComponentDescriptor descBase = new DataComponentDescriptor(desc_array, desc_length);
                            if(descBase.getData_component_id() == 0x08) // is isdbt closed catpion (ARIB STD-B24 ?�本 BML)
                            {
                                esiData.ttype = SUBTITLE_ARIB_STD_B24;
                                Log.d(TAG,"pid["+esiData.elementary_stream_pid+"] have ARIB_STD_B24 closed caption");
                            }
                        }
                    }break;
                }
            ...
            switch(temp_e.stream_type){
                ...
                case STREAM_MPEG2_VIDEO :
                    if (j < PMT_MAX_VIDEO_PID_NUM)
                    {
                        ...
                        //TODO show check the closed caption flag how to get
                        if(temp_e.C608_flag == 1)//centaur 20250304 add C608_flag
//                        if(mTunerType == TpInfo.ISDBT)
                        {
                            ...
                            pmt_data.subtitling_type[(int) pmt_data.subt_lang_num] = SubtitlingDescriptor.Subtitle.SUBTITLING_TYPE_CEA608;
                            pmt_data.subt_lang_num++;
                            pmt_data.subtitle = 1;
                        }
                    }
                    break;
                case STREAM_TELETEXT :
                    ...
                    else if(temp_e.ttype == SUBTITLE_ARIB_STD_B24)
                    {
                        if (pmt_data.subt_lang_num>=SUBT_MAX_LANG_NUM)
                            break;
                        pmt_data.subt_stream_id[(int) pmt_data.subt_lang_num] = temp_e.elementary_stream_pid;
                        pmt_data.subtitling_type[(int) pmt_data.subt_lang_num] = SubtitlingDescriptor.Subtitle.SUBTITLING_TYPE_ARIB_STD_B24;
                        pmt_data.subt_lang_num++;
                        pmt_data.subtitle = 1;
                        Log.d("gggg","get arib std b24 closed caption");
                    }
                    else if(k < PMT_MAX_TELETEXT_PID_NUM)
                    {
                        if(temp_e.ttype == TELETEXT_NO_SUBTITLE)
                        {
                            ...
                            Log.d(TAG,"Teletext_NO_SUBTITLE");
                        ...
                        else if(temp_e.ttype == TELETEXT_SUBTITLE)
                        {
                            ...
                            //Log.d(TAG,"Teletext_SUBTITLE"+ k);
                            if(k==1)
                            {
                                int subtitle_lang_count = 0;  // ✅ 初始化移到這裡
                                pmt_data.ttxt_init_page=100;
                                //							TABLE_TRACE_INFO("\ntemp_e->ttxt_lang_num[%d]",temp_e->ttxt_lang_num);
                                for (int idx = 0; idx < temp_e.ttxt_lang_num; idx++) {
                                    int type = temp_e.ttxt_type[idx];
                                    String lang = temp_e.ttxt_lang[idx];
                                    int mag = temp_e.ttxt_magazine_number[idx];
                                    int page = temp_e.ttxt_page_number[idx];
                                    // ?�是 Init Page，�??�為 init page（�?不�??��?言???�
                                    if (type == 0x01) {
                                        pmt_data.ttxt_init_page = mag * 100 + page;
                                    }

                                    if (type == 0x02 || type == 0x05) {
                                        boolean duplicate = false;

                                        for (int c = 0; c < subtitle_lang_count; c++) {
                                            if (pmt_data.ttxt_type[c] == type &&
                                                pmt_data.ttxt_lang[c].equals(lang) &&
                                                pmt_data.ttxt_magazine_number[c] == mag &&
                                                pmt_data.ttxt_page_number[c] == page) {
                                                duplicate = true;
                                                break;
                                            }
                                        }
                                        ...
                                        if (!duplicate) {
                                            int langIdx = subtitle_lang_count;

                                            pmt_data.ttxt_lang[langIdx] = lang;
                                            pmt_data.ttxt_type[langIdx] = type;
                                            pmt_data.ttxt_magazine_number[langIdx] = mag;
                                            pmt_data.ttxt_page_number[langIdx] = page;
                                            Log.d(TAG, String.format(
                                                            "TtxParse [Add] PID=%d Lang=%s Type=0x%02X Page=%d",
                                                            temp_e.elementary_stream_pid, lang, type, mag * 100 + page
                                                        ));
                                            subtitle_lang_count++;

                                            if (langIdx >= TTXT_MAX_LANG_NUM - 1)
                                        break;
                                }
                            }

                                }
                                pmt_data.ttxt_lang_num = subtitle_lang_count;
            ...
            if (temp_e.tdata == 0x66) {
                if (temp_e.stream_type == 0x0B) {
                    Log.d(TAG, String.format(
                            "DSM-CC Object Carousel found: PID=0x%04X (decimal %d)",
                            temp_e.elementary_stream_pid,
                            temp_e.elementary_stream_pid
                    ));
                    pmt_data.audio_pid[1] = temp_e.elementary_stream_pid; // legacy: UNT PID
                }
                else if (temp_e.stream_type == 0x0D) {
                    Log.d(TAG, String.format(
                            "DSM-CC U-N Messages found: PID=0x%04X (decimal %d)",
                            temp_e.elementary_stream_pid,
                            temp_e.elementary_stream_pid
                    ));
                }
                    ...
                    Log.d(TAG, String.format(
                            "DSM-CC unknown stream_type=0x%02X PID=0x%04X",
                            temp_e.stream_type,
                            temp_e.elementary_stream_pid
                    ));
                }

                pmt_data.update_pid = temp_e.elementary_stream_pid; // DSM-CC PID
                Log.d(TAG, String.format(
                        "PMT update_pid set to DSM-CC PID=0x%04X (stream_type=0x%02X)",
                        pmt_data.update_pid,
                        temp_e.stream_type
                ));
            }
    ...
    public void parsing(byte[] data, int lens) {
        ...
        try {
            ...
            System.arraycopy(data, 0, raw_data, 0, lens);
                ...
                byte [] raw_data_tmp= new byte[lens+raw_data_lens];
                System.arraycopy(raw_data, 0, raw_data_tmp, 0, raw_data_lens);
                System.arraycopy(data, 0, raw_data_tmp, raw_data_lens, lens);
                raw_data_lens = raw_data_lens + lens;
        ...
        if (!parseOk && mProgramMap == null) {
            Log.w(TAG, "PMT parsing failed; using empty Program_map");
            mProgramMap = new Program_map();
        }
    }
}
