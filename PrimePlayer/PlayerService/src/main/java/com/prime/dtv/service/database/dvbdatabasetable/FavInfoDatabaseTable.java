...
//            }
//        }
        mContext.getContentResolver().insert(CONTENT_URI, Value);
    }
...
//            }
//        }
        count = mContext.getContentResolver().update(CONTENT_URI, Value, where, selectionArgs);
    }
    ...
    public void remove(FavInfo favInfo) {
        ...
        count = mContext.getContentResolver().delete(CONTENT_URI, where, selectionArgs);
    }
    ...
    private List<FavInfo> queryList(int groupType) {
        ...
        Cursor cursor = mContext.getContentResolver().query(CONTENT_URI, null, selection, null, sort);
        List<FavInfo> favInfoList = new ArrayList<>();
        FavInfo favInfo = null;
        if(cursor != null) {
            if (cursor.moveToFirst()) {
