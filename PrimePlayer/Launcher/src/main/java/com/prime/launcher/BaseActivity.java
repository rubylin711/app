...
import com.prime.launcher.CustomView.ProgressDialog;
...
import com.prime.launcher.Home.Recommend.Activity.SideMenuActivity;
import com.prime.launcher.Home.Recommend.List.RecommendAdapter;
...
import com.prime.launcher.PVR.Conflict.AboutToStartDialog;
import com.prime.launcher.PVR.Conflict.ConflictDialog;
...
public abstract class BaseActivity extends AppCompatActivity implements PrimeBroadcastReceiver.Callback {
    ...
    private static final String EXTRA_TEMP_DISABLE_REC_SPACE_LIMIT_DIALOG
            = "com.prime.launcher.TEMP_DISABLE_REC_SPACE_LIMIT_DIALOG";
...
//                            getString(R.string.pvr_message_storage_not_enough_title));
    ...
    public void on_usb_mount(Intent intent) {
        ...
        if (totalSize < Pvcfg.getPvrHddTotalSizeLimit()) {
            ...
            gMsgDialog.show_panel("E621 " + context.getString(
                    ...
                    Pvcfg.getPvrHddTotalSizeLimit() / 1000));
        }
    }
    ...
    public void on_timer_change_channel(Intent intent) {
        ...
        String channelName = channel != null ? channel.getDisplayNum(MAX_LENGTH_OF_CHANNEL_NUM) + " " + channel.getDisplayName() : "NULL";
    ...
    private void cancel_all_timer(Context context, List<BookInfo> conflictBooks) {
        for (BookInfo book : conflictBooks)
            gPrimeDtv.cancel_alarms(context, gPrimeDtv.get_book_info_intent(book));
    }
    ...
    private void start_timer_record(BookInfo bookInfo, long delayMs) {
        CountDownTimer timer = new CountDownTimer(delayMs, 1000) {
            ...
            @Override public void onFinish() {
                ...
                gChChangeManager.pvr_record_start(bookInfo.getChannelId(), null, bookInfo.getDurationMs() / 1000, false);
            }
        }.start();
    ...
    private void stop_all_pvr() {
        ...
        if (gChChangeManager.is_timeshift_start()) {
