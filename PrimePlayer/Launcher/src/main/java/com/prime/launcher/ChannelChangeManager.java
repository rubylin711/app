...
public class ChannelChangeManager implements PrimeDtv.LauncherCallback {
...
//            Log.d(TAG,"tpInfo = "+tpInfo.ToString());
    ...
    * */
    public boolean pvr_record_start(long channelID, EPGEvent epgData, int duration, boolean isSeries) {
        Log.d(TAG, "pvr_record_start: channelID = " + channelID
                + ", event = " + epgData
                + ", duration = " + duration
                + ", isSeries = " + isSeries);
        int rec_tuner_id = 0;
        ...
        if (Utils.get_usb_space_info().get(1) < Pvcfg.getPvrHddLimitSize()) {
            Log.e(TAG, "pvr_record_start: not enough available space");
            return false;
        }
...
//        Log.e(TAG,"[Warning] This function not porting!! Please check");
        isFilePlayback = true;
        mPlaybackTunerId = g_dtv.pvr_PlayFileStart(recIdx, fromLastPosition, tunerId);
    ...
    private boolean pvr_clear_fcc(boolean isTimeshift) {
        ...
        if (tuner_id != null) {
            ...
            if(isTimeshift == true){
                ...
                if(Pvcfg.isDisableTableOnTimeshift()) {
                    g_dtv.stopAllEitTableOnCurrentTp(tuner_id);
                    g_dtv.stopMonitorTable(-1, tuner_id);
                }
                else
