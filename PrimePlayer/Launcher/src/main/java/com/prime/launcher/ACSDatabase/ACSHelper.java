package com.prime.launcher.ACSDatabase;

import static com.prime.launcher.Settings.fragment.EngineeringSettingFragment.INFO_HINT_OFF_VALUE;
...
import com.prime.launcher.Utils.DownloadImageSpeedTest;
...
public class ACSHelper {
    ...
    public static final String DO_FORCE_TUNE    = "com.prime.launcher.do_force_tune";
    ...
    public void init_project_id( Context context )
    {
        if(Pvcfg.getModuleType().equals(Pvcfg.LAUNCHER_MODULE.LAUNCHER_MODULE_CNS))
            return;
        String model_type = Pvcfg.LAUNCHER_MODULE.LAUNCHER_MODULE_DMG;
        if(get_ProjectId(context) == PROJECT_TBC) {
            model_type = Pvcfg.LAUNCHER_MODULE.LAUNCHER_MODULE_TBC;
        }
        ...
        Log.d(TAG,"acs command to set module to "+model_type);
