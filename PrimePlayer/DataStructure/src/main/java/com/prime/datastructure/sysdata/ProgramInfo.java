...
import static com.prime.datastructure.utils.Utils.MAX_LENGTH_OF_CHANNEL_NUM;
...
public class ProgramInfo implements Parcelable {
    ...
    public static final String TV_INPUT_CHANNEL_URI = "TvInputChannelUri";
    public static final String TIME_LOCK_FLAG = "TimeLockFlag";

    protected long ChannelId;
    protected int ServiceId;
    protected int Type; //is ALL_TV_TYPE , ALL_RADIO_TYPE
    protected int DisplayNum;
    protected String DisplayName = "";
    protected String ChName_eng;
    protected String ChName_chi;
    protected int Lock;
    protected int Skip;
    protected int PvrSkip;
    protected int CA;
    protected int TpId;
    protected int SatId;
    protected int Pcr;
    ...
    protected int AudioLRSelected;
    protected int AudioSelected;
    ...
    protected int TeletextInitPage;
    protected int TeletextPid;
    public List<TeletextInfo> pTeletext = new ArrayList<TeletextInfo>();
    protected int pmtPid;
    protected int pmtversion;
    protected int DeleteFlag;
    protected int TimeLockFlag;
    protected int AdultFlag;

    protected long category_type;
    protected ArrayList<Integer> service_product = new ArrayList<Integer>();
    protected byte [] mPmtRawData=null;
    protected Uri TvInputChannelUri;
    ...
    public ProgramInfo(ProgramInfo info){//eric lin add
        ...
        this.TvInputChannelUri = info.TvInputChannelUri;
        ...
        this.TeletextInitPage=info.TeletextInitPage;
        this.TeletextPid=info.TeletextPid;
        ...
        if(info.getPmtRawData() != null) {
            this.mPmtRawData = new byte[info.getPmtRawData().length];
            System.arraycopy(info.getPmtRawData(), 0, this.mPmtRawData, 0, info.getPmtRawData().length);
        }
    }
    ...
    public String ToString(){
        String info = "ServiceId = "+ServiceId+" Type = "+Type+" LCN = "+LCN+" DisplayNum = "+ DisplayNum +" DisplayName = "+ DisplayName
                ...
                +" AudioLRSelected = "+AudioLRSelected+" AudioLRSelected = "+AudioLRSelected + " TvInputChannelUri = "+TvInputChannelUri;
        if(pVideo != null){
        ...
        if(pSubtitle.size() >0){
            int i = 0 ;
            for( SubtitleInfo subtitleInfo : pSubtitle )
            {
                info += " subtitle AncPageId"+i+" = "+subtitleInfo.AncPageId;
                info += " subtitle ComPageId"+i+" = "+subtitleInfo.ComPageId;
                info += " subtitle Lang"+i+" = "+subtitleInfo.Lang;
                info += " subtitle Pid"+i+" = "+subtitleInfo.Pid;
                info += " subtitle Type"+i+" = "+subtitleInfo.Type;
                i++;
            }
        }
        ...
        if(pTeletext.size() >0){
            int i = 0 ;
            for( TeletextInfo teletextInfo : pTeletext )
            {
                info += " teletextInfo Lang"+i+" = "+teletextInfo.Lang;
                info += " teletextInfo MagazineNum"+i+" = "+teletextInfo.MagazineNum;
                info += " teletextInfo PageNum"+i+" = "+teletextInfo.PageNum;
                info += " teletextInfo Pid"+i+" = "+teletextInfo.Pid;
                info += " teletextInfo Type"+i+" = "+teletextInfo.Type;
                i++;
            }
        }
        return info;
    }
    public void setPmtRawData(byte [] pmtRawData) {
        mPmtRawData = pmtRawData;
    }

    public byte [] getPmtRawData() {
        return mPmtRawData;
    }
    ...
    public String getDisplayNumString() {
        int tunerType = Pvcfg.getTunerType();
        if(tunerType == TpInfo.ISDBT)
        {
            double value = DisplayNum / 100.0;
...
// 最多保留兩位整數（左邊補 0，超過兩位就保留最後兩位）
        ...
        }else if(tunerType == TpInfo.DVBT || (tunerType == TpInfo.DVBC))
        {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append(DisplayNum);

            for (int i = stringBuilder.length(); i < MAX_LENGTH_OF_CHANNEL_NUM; i++)
                stringBuilder.insert(0, "0");
    ...
    public String getDisplayNameFull() {
        return getDisplayNum(MAX_LENGTH_OF_CHANNEL_NUM) + " " + getDisplayName();
    }
    ...
    public void setTeletextInitPage(int page) {
        this.TeletextInitPage = page;
    }
    public int getTeletextInitPage() {
        return TeletextInitPage;
    }
     public void setTeletextPid(int pid) {
        this.TeletextPid = pid;
    }
...
//            this.PlayStreamType = programInfo.getPlayStreamType();
    ...
    public int getAdultFlag() {
        if(Pvcfg.getModuleType().equals(Pvcfg.LAUNCHER_MODULE.LAUNCHER_MODULE_DMG) || Pvcfg.getModuleType().equals(Pvcfg.LAUNCHER_MODULE.LAUNCHER_MODULE_TBC)){
            ...
            if(((category_type & 0x0800) >> 11) == 1) {//adult
                return 1;
        }
        return 0;
    }
    ...
    @ParcelablePlease
    public static class CaInfo implements Parcelable {
        protected int CaSystemId;
        protected int EcmPid;
        protected byte[] PrivateDataBytes;
        ...
        public void writeToParcel(Parcel dest, int flags) {
            ProgramInfo$CaInfoParcelablePlease.writeToParcel(this, dest, flags);
        }

        public static final Creator<CaInfo> CREATOR = new Creator<CaInfo>() {
            public CaInfo createFromParcel(Parcel source) {
                CaInfo target = new CaInfo();
                ProgramInfo$CaInfoParcelablePlease.readFromParcel(target, source);
                return target;
            }

            public CaInfo[] newArray(int size) {
                return new CaInfo[size];
            }
        };
    }
    ...
    public static class VideoInfo implements Parcelable {
        protected int PID;
        protected int Codec;
        public ArrayList<CaInfo> CaInfoList = new ArrayList<>();
...
//            Log.d(TAG, "getPrivateData: privatedata = " + Arrays.toString(privateData));
        ...
        @Override
        public void writeToParcel(Parcel dest, int flags) {
            ProgramInfo$VideoInfoParcelablePlease.writeToParcel(this, dest, flags);
        }

        public static final Creator<VideoInfo> CREATOR = new Creator<VideoInfo>() {
            public VideoInfo createFromParcel(Parcel source) {
                VideoInfo target = new VideoInfo();
                ProgramInfo$VideoInfoParcelablePlease.readFromParcel(target, source);
                return target;
            }
    ...
    public static class AudioInfo implements Parcelable {
        protected int Pid;
        protected int Codec;
        protected String LeftIsoLang;
        protected String RightIsoLang;
...
//            Log.d(TAG, "getPrivateData: privatedata = " + Arrays.toString(privateData));
        ...
        @Override
        public void writeToParcel(Parcel dest, int flags) {
            ProgramInfo$AudioInfoParcelablePlease.writeToParcel(this, dest, flags);
        }

        public static final Creator<AudioInfo> CREATOR = new Creator<AudioInfo>() {
            public AudioInfo createFromParcel(Parcel source) {
                AudioInfo target = new AudioInfo();
                ProgramInfo$AudioInfoParcelablePlease.readFromParcel(target, source);
                return target;
            }
    ...
    public static class SubtitleInfo implements Parcelable {
        ...
        protected int ComPageId;
        protected int AncPageId;
        ...
        @Override
        public void writeToParcel(Parcel dest, int flags) {
            ProgramInfo$SubtitleInfoParcelablePlease.writeToParcel(this, dest, flags);
        }
        ...
        public static final Creator<SubtitleInfo> CREATOR = new Creator<SubtitleInfo>() {
            public SubtitleInfo createFromParcel(Parcel source) {
                SubtitleInfo target = new SubtitleInfo();
                ProgramInfo$SubtitleInfoParcelablePlease.readFromParcel(target, source);
                return target;
            }
    ...
    @ParcelablePlease
    public static class TeletextInfo implements Parcelable {
        protected int Pid;
        protected int Type;
        protected String Lang;
        protected int MagazineNum;
        ...
        @Override
        public void writeToParcel(Parcel dest, int flags) {
            ProgramInfo$TeletextInfoParcelablePlease.writeToParcel(this, dest, flags);
        }

        public static final Creator<TeletextInfo> CREATOR = new Creator<TeletextInfo>() {
            public TeletextInfo createFromParcel(Parcel source) {
                TeletextInfo target = new TeletextInfo();
                ProgramInfo$TeletextInfoParcelablePlease.readFromParcel(target, source);
                return target;
            }

            public TeletextInfo[] newArray(int size) {
                return new TeletextInfo[size];
            }
        };
    }
    ...
    protected VideoInfo videoInfoDeserialize(String data) {
...
//        Gson gson = new Gson();
    ...
    protected List<AudioInfo> audioInfoDeserialize(String data) {
        ...
        for (String entry : entries) {
            String[] parts = entry.split("#", 2); // [0] ?�基?��?�? [1] ??CaInfo 清單（可?��?
...
//        Gson gson = new Gson();
    ...
    public static List<SubtitleInfo> subtitleListDeserialize(String data) {
        ...
        String[] entries = data.split("\\|");
        for (String entry : entries) {
            String[] fields = entry.split(",", -1); // 保�?空�?
            if (fields.length >= 5) {
...
//        Gson gson = new Gson();
    ...
    public static List<TeletextInfo> teletextListDeserialize(String data) {
        ...
        String[] entries = data.split("\\|");
        for (String entry : entries) {
            String[] fields = entry.split(",", -1); // -1 保�?空�?�?
            if (fields.length >= 5) {
...
//        Type collectionType = new TypeToken<List<TeletextInfo>>() {}.getType();
    ...
    public Uri getTvInputChannelUri() {
        return TvInputChannelUri;
    }

    public void setTvInputChannelUri(Uri uri) {
        TvInputChannelUri = uri;
    }
    ...
    @Override
    public void writeToParcel(Parcel dest, int flags) {
        ProgramInfoParcelablePlease.writeToParcel(this, dest, flags);
    }
    ...
    public static final Creator<ProgramInfo> CREATOR = new Creator<ProgramInfo>() {
        public ProgramInfo createFromParcel(Parcel source) {
            ...
            ProgramInfoParcelablePlease.readFromParcel(target, source);
            return target;
        }
    ...
    public boolean is_4k(ProgramInfo programInfo) {
        if (null == programInfo)
            return false;
