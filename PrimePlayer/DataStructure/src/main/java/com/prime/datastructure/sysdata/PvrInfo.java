...
public class PvrInfo {
    ...
    public enum EnSeekMode {
        ...
        public static EnSeekMode fromValue(int value) {
            for (EnSeekMode mode : EnSeekMode.values()) {
                if (mode.value == value) {
                    return mode;
                }
            }

            return PLAY_SEEK_SET;
        }
    }
    ...
    public enum EnPlaySpeed {
        ...
        public static EnPlaySpeed convertSpeed(int speed){
        ...
        public static EnPlaySpeed fromFloatSpeed(float speed){
            // Forward
            if (speed == 1.0f) return PLAY_SPEED_ID_FWD01;
            if (speed == 2.0f) return PLAY_SPEED_ID_FWD02;
            if (speed == 4.0f) return PLAY_SPEED_ID_FWD04;
            if (speed == 8.0f) return PLAY_SPEED_ID_FWD08;
            if (speed == 16.0f) return PLAY_SPEED_ID_FWD16;
            if (speed == 32.0f) return PLAY_SPEED_ID_FWD32;
            if (speed == 64.0f) return PLAY_SPEED_ID_FWD64;
            if (speed == 128.0f) return PLAY_SPEED_ID_FWD128;

            // Rewind/Reverse
            if (speed == -1.0f) return PLAY_SPEED_ID_REV01;
            if (speed == -2.0f) return PLAY_SPEED_ID_REV02;
            if (speed == -4.0f) return PLAY_SPEED_ID_REV04;
            if (speed == -8.0f) return PLAY_SPEED_ID_REV08;
            if (speed == -16.0f) return PLAY_SPEED_ID_REV16;
            if (speed == -32.0f) return PLAY_SPEED_ID_REV32;
            if (speed == -64.0f) return PLAY_SPEED_ID_REV64;
            if (speed == -128.0f) return PLAY_SPEED_ID_REV128;

            // 0.0f
            if (speed == 0.0f) return PLAY_SPEED_ID_ZERO;
        ...
        public static float toFloatSpeed(int enumValue) {
            // Forward
            if (enumValue == PLAY_SPEED_ID_FWD01.getValue()) return 1.0f;
            if (enumValue == PLAY_SPEED_ID_FWD02.getValue()) return 2.0f;
            if (enumValue == PLAY_SPEED_ID_FWD04.getValue()) return 4.0f;
            if (enumValue == PLAY_SPEED_ID_FWD08.getValue()) return 8.0f;
            if (enumValue == PLAY_SPEED_ID_FWD16.getValue()) return 16.0f;
            if (enumValue == PLAY_SPEED_ID_FWD32.getValue()) return 32.0f;
            if (enumValue == PLAY_SPEED_ID_FWD64.getValue()) return 64.0f;
            if (enumValue == PLAY_SPEED_ID_FWD128.getValue()) return 128.0f;

            // Rewind/Reverse
            if (enumValue == PLAY_SPEED_ID_REV01.getValue()) return -1.0f;
            if (enumValue == PLAY_SPEED_ID_REV02.getValue()) return -2.0f;
            if (enumValue == PLAY_SPEED_ID_REV04.getValue()) return -4.0f;
            if (enumValue == PLAY_SPEED_ID_REV08.getValue()) return -8.0f;
            if (enumValue == PLAY_SPEED_ID_REV16.getValue()) return -16.0f;
            if (enumValue == PLAY_SPEED_ID_REV32.getValue()) return -32.0f;
            if (enumValue == PLAY_SPEED_ID_REV64.getValue()) return -64.0f;
            if (enumValue == PLAY_SPEED_ID_REV128.getValue()) return -128.0f;

            // 0.0f
            if (enumValue == PLAY_SPEED_ID_ZERO.getValue()) return 0.0f;

            // slow?

            // not found, return 1.0f
            return 1.0f;
        }
    }
