...
public class TimerThread extends Thread{
    ...
    private void handl_wvcas_ref_casdata_time(){
        ...
        if(mDataManager.getGposInfo().getSTBRefCasDataTime() == 0)
            return;
        if(mRunningTime >= (mDataManager.getGposInfo().getSTBRefCasDataTime()*60)){
            LogUtils.d(" call  PESI_EVT_CA_WIDEVINE_REFRESH_CAS_DATA mRunningTime = "+mRunningTime+" getSTBRefCasDataTime = "+mDataManager.getGposInfo().getSTBRefCasDataTime()*60);
            mRunningTime = 0;
            if(Pvcfg.isRunRefreshCasByTime())
                mCallback.sendCallbackMessage(DTVMessage.PESI_EVT_CA_WIDEVINE_REFRESH_CAS_DATA, 0 , 0, mDataManager.getGposInfo().getCurChannelId());
            return;
        }
        mRunningTime ++;
    }
