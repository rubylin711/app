...
import com.prime.dmg.launcher.ACSDatabase.ACSForceSrceenParam;
...
public class ForceScreenActivity extends BaseActivity {
    ...
    public static final String ACTION_CLOSE_FORCE_SCREEN = "com.prime.dmg.launcher.action.CLOSE_FORCE_SCREEN";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(param.getQrCode()!=null&&!param.getQrCode().isEmpty()) {
            Bitmap qr_code = URLUtils.generate_qr_code(MailInfo.get_mail_qrcode_type(mContext, param.getQrCode()), 600) ;
            if (!isFinishing() && !isDestroyed())
                Glide.with(this)
                        .load(qr_code)
