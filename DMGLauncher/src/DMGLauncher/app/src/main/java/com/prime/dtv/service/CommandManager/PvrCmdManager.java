...
public class PvrCmdManager extends BaseManager {
    ...
    public int pvrRecordStart(PvrRecStartParam startParam,int tunerId){
        ...
        recIdx = new PvrDataManager(mContext).getAvailableIndex(isSeries, startParam.getSeriesKey());
        recPath = new PvrDataManager(mContext).preCreateDirectories(isSeries, recIdx);
        ...
        dbRecordInfo.setRating(startParam.getmProgramInfo().getParentalRating());
...
//        }
    ...
    public boolean pvrPlayCheckLastPosiitonPoint(PvrRecIdx recIdx) {
        Log.d(TAG,"pvrPlayCheckLastPosiitonPoint, recIdx="+recIdx);
...
//        }
    ...
    public int pvrPlaySeek(int playId, PvrInfo.EnSeekMode seekMode, int offsetSec){
        Log.d(TAG,"pvrPlaySeek, playId = " + playId);
        int seek;
...
//        }
    ...
    public int pvrGetRecCount(){
        ...
        int totalRecCount;
        totalRecCount = new PvrDataManager(mContext).getMasterTotalRecordCount();
    ...
    public int pvrGetSeriesRecCount(int masterIndex){
        ...
        int seriesRecCount;
        seriesRecCount = new PvrDataManager(mContext).getSeriesRecordTotalCount(masterIndex);
    ...
    public List<PvrRecFileInfo> pvrGetRecLink(int startIndex, int count){
        ...
        pvrDbRecordInfoLlist = new PvrDataManager(mContext).getMasterFilesInfoList(startIndex,count);
    ...
    public List<PvrRecFileInfo> pvrGetSeriesRecLink(PvrRecIdx tableKeyInfo, int count){
        ...
        List<PvrRecFileInfo>   pvrRecFileInfoList = new ArrayList<PvrRecFileInfo>();
        List<PvrDbRecordInfo>  pvrDbRecordInfoLlist;
        pvrDbRecordInfoLlist = new PvrDataManager(mContext).getSeriesFilesInfoList(tableKeyInfo.getMasterIdx(),tableKeyInfo.getSeriesIdx(),count);
        for (PvrDbRecordInfo tmpDbRecordInfo : pvrDbRecordInfoLlist) {
            PvrRecFileInfo tmpRecFileInfo= new PvrRecFileInfo(tmpDbRecordInfo);
            pvrRecFileInfoList.add(tmpRecFileInfo);
        }
    ...
    public int pvrCheckSeriesComplete(byte[]  seriesKey){
        Log.d(TAG,"pvrCheckSeriesEpisode, seriesKey="+seriesKey);
        return new PvrDataManager(mContext).pvrDB_checkSeriescomplete(seriesKey);
    }
