...
public class PvrDataManager {
    ...
    private void updatePlaybackTable(String columnName, long newValue, int masterIdx) {
        ContentValues values = new ContentValues();
        values.put(columnName, newValue);

        String where = PvrInfoDatabaseTable.MASTER_ID + " = ?";
        String[] whereArgs = { String.valueOf(masterIdx) };

        try {
            int rowsUpdated = resolver.update(PvrInfoDatabaseTable.CONTENT_PLAYBACK_URI, values, where, whereArgs);
            if (rowsUpdated > 0) {
    ...
    private void updateMasterRecord(String columnName, int newValue, int masterIdx) {
        ...
        try {
            int rowsUpdated = resolver.update(PvrInfoDatabaseTable.CONTENT_MASTER_URI, values, where, whereArgs);
            if (rowsUpdated > 0) {
                // Log.d(TAG, "Successfully updated " + columnName + " for masterId " + masterId);
            } else {
                // Log.d(TAG, "Failed to update " + columnName + " for masterId " + masterId);
    ...
    private Uri insertPlaybackTable(PvrRecFileInfo pvrRecFileInfo) {
        ...
        if(playbackCount>=30) {//playback list more than 30, the oldest one will be deleted
            Cursor cursor = resolver.query(
                    PvrInfoDatabaseTable.CONTENT_PLAYBACK_URI,
                    new String[]{PvrInfoDatabaseTable.MASTER_ID},
                    null, null,
                    PvrInfoDatabaseTable.PLAYBACK_TIME + " ASC LIMIT 1"
            );

            if (cursor != null && cursor.moveToFirst()) {
                long idToDelete = cursor.getLong(cursor.getColumnIndexOrThrow(PvrInfoDatabaseTable.MASTER_ID));
                cursor.close();

                resolver.delete(ContentUris.withAppendedId(PvrInfoDatabaseTable.CONTENT_PLAYBACK_URI, idToDelete), null, null);
            }
        }
        Gson gson = new Gson();
        ContentValues values = new ContentValues();
        ...
        values.put(PvrInfoDatabaseTable.SERIES, pvrRecFileInfo.isSeries() ? 1 : 0);//convert bool into int and and store it in SQLite
        values.put(PvrInfoDatabaseTable.PLAY_TIME, pvrRecFileInfo.getPlaybackTime());

        return resolver.insert(PvrInfoDatabaseTable.CONTENT_PLAYBACK_URI, values);
    }
    private Uri insertMasterRecord(PvrDbRecordInfo dbRecordInfo) {
        ...
        values.put(PvrInfoDatabaseTable.EVENT_NAME, dbRecordInfo.getEventName());
        ...
        return resolver.insert(PvrInfoDatabaseTable.CONTENT_MASTER_URI, values);
    }
    ...
    private Uri insertSeriesRecord(PvrDbRecordInfo dbRecordInfo) {
        ...
        Gson gson = new Gson();
        Uri seriesTableUri = Uri.parse(PvrInfoDatabaseTable.CONTENT_SERIES_STRING + masterId);
        ContentValues values = new ContentValues();
    ...
    private void updateSeriesRecord(String columnName, int newValue, PvrRecIdx pvrRecIdx) {
        ContentValues values = new ContentValues();
        values.put(columnName, newValue);
        Uri seriesTableUri = Uri.parse(PvrInfoDatabaseTable.CONTENT_SERIES_STRING + pvrRecIdx.getMasterIdx());
    ...
    private PvrDbRecordInfo querySeries(Context context, int masterIdx, int seriesIdx) {
        ...
        try {
            Uri CONTENT_SERIES_URI = Uri.parse(PvrInfoDatabaseTable.CONTENT_SERIES_STRING + masterIdx);
            String[] projection = {
    ...
    private PvrRecFileInfo queryPlayback(Context context, int masterIdx, int seriesIdx) {
        ...
        try {
            ...
            String selection = PvrInfoDatabaseTable.MASTER_ID + " = ?";

            String[] selectionArgs = {String.valueOf(masterIdx)};

            cursor = resolver.query(PvrInfoDatabaseTable.CONTENT_PLAYBACK_URI, projection, selection, selectionArgs, null);
            ...
            if (cursor != null && cursor.moveToFirst()) {
                ...
                pvrRecFileInfo.setIsSeries(mDbUtils.GetIntFromTable(cursor,PvrInfoDatabaseTable.SERIES) == 1);//convert int to bool: 1 -> true, 0 -> false
    ...
    {
        ArrayList<PvrDbRecordInfo> dvrDbRecordInfoList = new ArrayList<PvrDbRecordInfo>();
        Cursor cursor = null;
        try {
            Uri uri = PvrInfoDatabaseTable.CONTENT_MASTER_URI;
            String[] projection = {
    ...
    public ArrayList<PvrDbRecordInfo> getPlaybackFilesInfoList(int startIndex, int count)
    {
        ...
        Cursor cursor = null;
        try {
            Uri uri = PvrInfoDatabaseTable.CONTENT_PLAYBACK_URI;
            String[] projection = {
                    PvrInfoDatabaseTable.MASTER_ID, PvrInfoDatabaseTable.SERIES_ID,
    ...
    public int addDataToPlaybackTable(PvrRecFileInfo pvrRecFileInfo)
    {
        ...
        if (playbackRecFileInfo == null)
        {
            ...
            updatePlaybackTable(PvrInfoDatabaseTable.PLAYBACK_TIME, pvrRecFileInfo.getPlaybackTime(), pvrRecFileInfo.getMasterIdx());
        }
        return 0;
    }
    public int addDataToTable(PvrDbRecordInfo dbRecordInfo) {
        ...
        if (dbRecordInfo.getIsSeries() == true) {
    ...
    public int modifyRecordStatus(PvrRecIdx pvrRecIdx, int recordStatus){
        ...
        if (pvrRecIdx.getSeriesIdx() == 0xffff) {
            ...
            if(pvrRecIdx.getSeriesIdx() == 0) {
                updateMasterRecord(PvrInfoDatabaseTable.RECORD_STATUS, recordStatus, pvrRecIdx.getMasterIdx());
            }
            updateSeriesRecord(PvrInfoDatabaseTable.RECORD_STATUS, recordStatus, pvrRecIdx);
        }
        return 0;
    }
    ...
    public int getMasterTotalErrorRecordCount(){
        ...
        String selection = "RECORD_STATUS = ?";
    ...
    public int getSeriesRecordTotalCount(int masterIdx){
        int count = 0;
        Uri seriesTableUri = Uri.parse(PvrInfoDatabaseTable.CONTENT_SERIES_STRING + masterIdx);
        Cursor cursor = resolver.query(
                seriesTableUri,
                new String[]{"COUNT(*) AS count"},
                null,
    ...
    public int getSeriesMaxPrimaryId(int masterIdx) {
        ...
        try {
            //Uri uri = PvrInfoDatabaseTable.CONTENT_MASTER_URI;
            Uri uri = Uri.parse(PvrInfoDatabaseTable.CONTENT_SERIES_STRING + masterIdx);
    ...
    public int getSeriesErrorRecordTotalCount(int masterIdx){
        ...
        String selection = "SERIES_RECORD_STATUS = ?";
        String[] selectionArgs = { "0" };
    ...
    public int deleteAllRecordFiles(){
        ...
        try {
            ...
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    ...
                    if (isSeries == 1) {
                        ...
                        Uri series_uri = Uri.parse(PvrInfoDatabaseTable.CONTENT_SERIES_STRING + masterIdx);
                        String[] projection_series = {
                                PvrInfoDatabaseTable.SERIES_PRIMARY_ID
                        };
                        seriesCursor = resolver.query(series_uri, projection_series, null, null, null);
                        if (seriesCursor != null && seriesCursor.moveToFirst()) {
                            do {
                                ...
                                String filePath_series = usbPath + dirPath_series;
                                deleteFileOrDirectory(filePath_series);
                                Log.d(TAG, "Deleted Series File: " + filePath_series);
                            } while (seriesCursor.moveToNext());
                            seriesCursor.close();
                        }

                        Uri dropSeriesUri = Uri.parse(PvrInfoDatabaseTable.CONTENT_STRING + PvrDatabase.DROP_SERIES_TABLE +"/" +masterIdx);
            ...
            Uri dropMasterUri = Uri.parse(PvrInfoDatabaseTable.CONTENT_STRING + PvrDatabase.DROP_MASTER_TABLE);
            resolver.delete(dropMasterUri, null, null);
            //drop playback list
            Uri dropPlaybackUri = Uri.parse(PvrInfoDatabaseTable.CONTENT_STRING + PvrDatabase.DROP_MASTER_TABLE);
            resolver.delete(dropPlaybackUri, null, null);
    ...
    private int deleteOneSeriesRecord(PvrRecIdx pvrRecIdx) {
        ...
        int masterIdx = pvrRecIdx.getMasterIdx();
        int seriesIdx = pvrRecIdx.getSeriesIdx();
        Uri uriSeries = Uri.parse(PvrInfoDatabaseTable.CONTENT_SERIES_STRING + masterIdx);
        ...
        if (getSeriesRecordTotalCount(masterIdx) == 0) {
            Uri dropSeriesUri = Uri.parse(
                    PvrInfoDatabaseTable.CONTENT_STRING
    ...
    public int deleteOnePlaybackInList(PvrRecIdx pvrRecIdx){
        ...
        int masterIdx = pvrRecIdx.getMasterIdx();
        int seriesIdx = pvrRecIdx.getSeriesIdx();
        Uri playback_uri = PvrInfoDatabaseTable.CONTENT_PLAYBACK_URI;
        ...
        try {
            ...
            if (masterCursor != null) {
                while (masterCursor.moveToNext()) {
                    ...
                    if (isSeries == 1) {
                        Cursor seriesCursor = null;
                        Uri seriesUri = Uri.parse(PvrInfoDatabaseTable.CONTENT_SERIES_STRING + masterIdx);
    ...
    public int modifyPlaybacktime(PvrRecIdx pvrRecIdx, int playbackTime){
        ...
        updatePlaybackTable(PvrInfoDatabaseTable.PLAYBACK_TIME, playbackTime, pvrRecIdx.getMasterIdx());
        return 0;
    }
    ...
    //Find out if a certain episode of a TV series has been recorded
    public int querySeriesEpisode(byte[] seriesKey, int episode){
        ...
        if(masterIdx!=-1)
        {
            ...
            try {
                ...
                String[] selectionArgs = {String.valueOf(episode)};
                cursor = resolver.query(CONTENT_SERIES_URI, projection, selection, selectionArgs, null);
                if (cursor != null && cursor.moveToFirst()) {
                    find_episode = 0;
                }
    ...
    public int pvrDB_checkSeriescomplete(byte[] seriesKey){
        int find_totalEpisode = -1;
        int totalEpisode = 0;
        int masterIdx = getMasterIdxBasedOnSeriesKey(seriesKey);
        if(masterIdx!=-1) {
            Cursor cursor = null;
            ...
            if(totalEpisode!=0)
            {
                int seriesRecordCount = getSeriesRecordTotalCount(masterIdx);
                if(totalEpisode == seriesRecordCount)
                {
                    find_totalEpisode = 0;
                }
            }
        }
        return find_totalEpisode;
    }
}
