...
//                Log.d(TAG, "SetCurChannel: i = " + i + " CurChannelList " + CurChannelList.get(i).getChannelId() + " pChannel " + pChannel.getChannelId());
                if (CurChannelList.get(i).get_channel_id() == pChannel.get_channel_id()) {
                    ChannelIndex = i;
                }
            }
        }
        ...
        if(pChannel != null && CurChannelList != null) {
            ...
            if(pCurGroupType >= FavGroup.ALL_TV_TYPE && pCurGroupType < FavGroup.ALL_RADIO_TYPE) {
                //Log.d(TAG, "SetCurChannel: TV ChannelIndex = " + ChannelIndex);
                TvChannelId = CurChannelList.get(ChannelIndex).get_channel_id();//Scoty 20180810 change save Tv/Radio channel by channelId
            }
            else if(pCurGroupType >= FavGroup.ALL_RADIO_TYPE && pCurGroupType < FavGroup.ALL_TV_RADIO_TYPE_MAX) {
                //Log.d(TAG, "SetCurChannel: RADIO ChannelIndex = " + ChannelIndex);
                RadioChannelId = CurChannelList.get(ChannelIndex).get_channel_id();//Scoty 20180810 change save Tv/Radio channel by channelId
            }
        ...
        if(pChannel != null) {
            ...
            if (PreChannelList != null) {
                for (int i = 0; i < PreChannelList.size(); i++) {
                    if (pChannel != null && pChannel.get_channel_id() == PreChannelList.get(i).get_channel_id()) {
                        PreChannelIndex = i;
                    }
                }
            }
...
//    }
    ...
    public void setPipChannelUp()
    {
        ...
        long CurChannelChannelId = CurChannelList.get(CurChannelIndex).get_channel_id();
        int CurChannelTpId = CurChannelList.get(CurChannelIndex).get_tp_id();
        if(PipChannelIndex == -1)
            PipChannelIndex = 0;
        ...
        if(PipChannelIndex >= (CurChannelList.size()-1))
        {
            ...
            for (int i = 0; i < CurChannelList.size(); i++)
            {
                if (CurChannelList.get(i).get_tp_id() == CurChannelTpId &&
                        CurChannelList.get(i).get_channel_id() != CurChannelChannelId) {
                    PipChannelIndex = i;
                    break;
                }
            }
    ...
    public void setChannelUp() {
        ...
        if(CurChannelIndex >= (CurChannelList.size()-1)) {
            CurChannelIndex = 0;
            while(CurChannelList.get(CurChannelIndex).get_pvr_skip() == 1)
                CurChannelIndex++;
            //Scoty 20180810 change save Tv/Radio channel by channelId -s
            if(CurGroupType >= FavGroup.ALL_TV_TYPE && CurGroupType < FavGroup.ALL_RADIO_TYPE) {
                TvChannelId = CurChannelList.get(0).get_channel_id();
            } else if (CurGroupType >= FavGroup.ALL_RADIO_TYPE && CurGroupType < FavGroup.ALL_TV_RADIO_TYPE_MAX) {
                RadioChannelId = CurChannelList.get(0).get_channel_id();
            }
            ...
            //Scoty 20180810 change save Tv/Radio channel by channelId -s
            if(CurGroupType >= FavGroup.ALL_TV_TYPE && CurGroupType < FavGroup.ALL_RADIO_TYPE) {
                TvChannelId = CurChannelList.get(CurChannelIndex).get_channel_id();
            } else if (CurGroupType >= FavGroup.ALL_RADIO_TYPE && CurGroupType < FavGroup.ALL_TV_RADIO_TYPE_MAX) {
                RadioChannelId = CurChannelList.get(CurChannelIndex).get_channel_id();
            }
            //Scoty 20180810 change save Tv/Radio channel by channelId -e
        }
    ...
    public int getCurListPos(long channelId) {
        if(CurChannelList != null) {
            ...
            for (int i = 0; i < CurChannelList.size(); i++) {
                if(CurChannelList.get(i).get_channel_id() == channelId)
                    return i;
            }
        }
        return -1;
    ...
    public long getFastPreChId()
    {
        ...
        if(CurGroupType >= FavGroup.ALL_TV_TYPE && CurGroupType < FavGroup.ALL_RADIO_TYPE) {
            if (index <= 0) {
                ...
                while (CurChannelList.get(index).get_pvr_skip() == 1) {
                    index--;
                }
            } else {
                index--;
            ...
            return CurChannelList.get(index).get_channel_id();
        }
        else {//radio
            return 0;
        }
