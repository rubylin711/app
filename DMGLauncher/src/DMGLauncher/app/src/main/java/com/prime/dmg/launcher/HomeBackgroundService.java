...
import com.prime.dmg.launcher.Settings.ChannelPreScanningActivity;
...
import com.prime.dmg.launcher.OTA.OtaParam;
...
//            Log.d(TAG,"HomeActivity.isActivityAlive() = "+isAppAlive);
            if(acsHelper.check_asc_command(context,intent,SERVICE_CALL) == 1) {
                ...
                else if ( isChannelScan(intent) )
                {
                    ...
                    {
                        symbol_rate_str = (Pvcfg.getModuleType() == Pvcfg.MODULE_DMG?ChannelPreScanningActivity.DEFAULT_DMG_CHANNEL_SYMBOL_RATE:ChannelPreScanningActivity.DEFAULT_CHANNEL_SYMBOL_RATE);
                    }
                    if ( frequency_str == null )
                    {
                        frequency_str = (Pvcfg.getModuleType() == Pvcfg.MODULE_DMG?ChannelPreScanningActivity.DEFAULT_DMG_CHANNEL_FREQUENCY:ChannelPreScanningActivity.DEFAULT_CHANNEL_FREQUENCY);
                    }
                    if ( qam_str == null )
                    {
                        qam_str = (Pvcfg.getModuleType() == Pvcfg.MODULE_DMG?ChannelPreScanningActivity.DEFAULT_DMG_CHANNEL_QAM_TEXT:ChannelPreScanningActivity.DEFAULT_CHANNEL_QAM_TEXT);
                    }
