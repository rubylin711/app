                ...
                if (cryptoUtils.verifyECDSASignature(sourceData, signatureData) == true) {
                    ...
                    for (i = 0; i < mMpsDataUnitCount; i++) {
                        ...
                        if (lens > total_len) {
                            ...
                            index = index + 1;
                            mpsDataUnit.mEncDataLen = Utils.getInt(data, index, 1, Utils.MASK_8BITS);
...
*/
                                ...
                                mpsDataUnit.mMpsData.mCommandType = Utils.getInt(tmp_data2, index_tmp, 2, Utils.MASK_16BITS);
                                if(mpsDataUnit.mMpsData.mCommandType != 0xA001)
                                {
                                    continue;
                                }
                                index_tmp = index_tmp + 2;
                                mpsDataUnit.mMpsData.mCommandLength = Utils.getInt(tmp_data2, index_tmp, 2, Utils.MASK_16BITS);
                                index_tmp = index_tmp + 2;
                                if(mpsDataUnit.mMpsData.mCommandLength >= 13) {
                                    System.arraycopy(tmp_data2, index_tmp, mpsDataUnit.mMpsData.mZipCode, 0, ZIP_CODE_LENGTH);
                                    index_tmp = index_tmp + ZIP_CODE_LENGTH;
                                    System.arraycopy(tmp_data2, index_tmp, mpsDataUnit.mMpsData.mStbModel, 0, STB_MODEL_LENGTH);
                                    index_tmp = index_tmp + STB_MODEL_LENGTH;
                                    mpsDataUnit.mMpsData.mDelete = Utils.getInt(tmp_data2, index_tmp, 1, Utils.MASK_8BITS);
                                    index_tmp = index_tmp + 1;
                                    mpsDataUnit.mMpsData.mMaxDelaySec = Utils.getInt(tmp_data2, index_tmp, 2, Utils.MASK_16BITS);
                                    index_tmp = index_tmp + 2;
                                }
                                //index=index+index_tmp;
                            }
                            else {
                                int tmpLen=total_len;
                                tmpLen=tmpLen+index;
                                if(lens > 4 + tmpLen) {
                                    mpsDataUnit.mMpsData = new MpsData();
                                    mpsDataUnit.mMpsData.mCommandType = Utils.getInt(data, index, 2, Utils.MASK_16BITS);
                                    if(mpsDataUnit.mMpsData.mCommandType != 0xA001)
                                    {
                                        continue;
                                    }
                                    index = index + 2;
                                    mpsDataUnit.mMpsData.mCommandLength = Utils.getInt(data, index, 2, Utils.MASK_16BITS);
                                    index = index + 2;
                                    System.arraycopy(data, index, mpsDataUnit.mMpsData.mZipCode, 0, ZIP_CODE_LENGTH);
                                    index = index + ZIP_CODE_LENGTH;
                                    System.arraycopy(data, index, mpsDataUnit.mMpsData.mStbModel, 0, STB_MODEL_LENGTH);
                                    index = index + STB_MODEL_LENGTH;
                                    mpsDataUnit.mMpsData.mDelete = Utils.getInt(data, index, 1, Utils.MASK_8BITS);
                                    index = index + 1;
                                    mpsDataUnit.mMpsData.mMaxDelaySec = Utils.getInt(data, index, 2, Utils.MASK_16BITS);
                                    index = index + 2;
                                }
                            }
    ...
    private void printfMessage(){
        ...
        for(i=0;i<mMpsDataUnitList.size();i++) {
            ...
            if (mMpsDataUnitList.get(i).mMpsData != null) {
                LogUtils.d("LLLLLL mMpsData.mCommandType = " + mMpsDataUnitList.get(i).mMpsData.mCommandType);
                LogUtils.d("LLLLLL mMpsData.mCommandLength = " + mMpsDataUnitList.get(i).mMpsData.mCommandLength);
                LogUtils.d("LLLLLL mMpsData.mZipCode = "+ bytesToHexWithSpaces(mMpsDataUnitList.get(i).mMpsData.mZipCode));
                LogUtils.d("LLLLLL mMpsData.mStbModel = "+ bytesToHexWithSpaces(mMpsDataUnitList.get(i).mMpsData.mStbModel));
                LogUtils.d("LLLLLL mMpsData.mDelete = " + mMpsDataUnitList.get(i).mMpsData.mDelete);
                LogUtils.d("LLLLLL mMpsData.mMaxDelaySec = " + mMpsDataUnitList.get(i).mMpsData.mMaxDelaySec);
            }
        }
    }
}
